...
use neoengram_central::{
    ...
    RevokeVolumeForLifecycleRequest, SqliteAuthorityConfig, StorageEnrollmentAccessMode,
...
use neoengram_domain::protocol::{
    ...
    AgentWorkloadCertificateDer, DeletionId, Ed25519PublicKeySpki, Ed25519Signature, EdgeClusterId,
    Extensions, LifecycleGeneration, MountAccessMode, MountGeneration, OwnerGeneration,
...
async fn one_volume_replacement_matches_in_memory_and_sqlite() {
    ...
    let bytes = std::fs::read(directory.path().join("authority.sqlite3")).unwrap();
...
fn sqlite_registry_config_debug_redacts_physical_path() {
    let config = SqliteAuthorityConfig::new("/secret/registry/location");
...
#[tokio::test]
async fn authority_integrity_check_includes_registry_tables() {
    let directory = TempDir::new().unwrap();
...
async fn token_scope_and_bootstrap_request_identity_are_bound() {
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(wrong_token))
            .await
            .unwrap_err()
            .code(),
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(wrong_scope))
            .await
            .unwrap_err()
            .code(),
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(wrong_tenant))
            .await
            .unwrap_err()
            .code(),
        CentralErrorCode::AgentIdentityMismatch
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(wrong_descriptor))
            .await
            .unwrap_err()
            .code(),
        CentralErrorCode::AgentIdentityMismatch
    ...
    let accepted = service
        .bootstrap_agent_with_proof(initial_bootstrap_request())
        .await
        .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(reused_request))
            .await
            .unwrap_err()
            .code(),
        CentralErrorCode::AgentIdentityMismatch
...
async fn approval_fails_closed_on_unsafe_probe_but_allows_degraded_health() {
    ...
    for probe in unsafe_probes {
        ...
        let pending = service
            .bootstrap_agent_with_proof(resign_bootstrap_request(bootstrap))
            .await
            .unwrap();
        assert_eq!(
            service
...
async fn assert_immutable_identity(repository: Arc<dyn AgentRegistryRepository>) {
    ...
    tampered.resource_version = neoengram_domain::protocol::ResourceVersion::new(previous + 1);
...
async fn run_registry_identity_conflicts(repository: Arc<dyn AgentRegistryRepository>) {
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(reused_bootstrap_id))
            .await
            .unwrap_err()
            .code(),
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(reused_installation))
            .await
            .unwrap_err()
            .code(),
        CentralErrorCode::AgentIdentityMismatch
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(reused_key))
            .await
            .unwrap_err()
            .code(),
        CentralErrorCode::AgentIdentityMismatch
    ...
    let independent_pending = service
        .bootstrap_agent_with_proof(independent_bootstrap_request())
        .await
        .unwrap();
...
#[tokio::test]
async fn volume_lifecycle_revocation_fences_owner_and_replays() {
    let memory = Arc::new(InMemoryAgentRegistry::new());
    run_volume_lifecycle_revocation(memory.clone()).await;

    let directory = TempDir::new().unwrap();
    let sqlite = open_sqlite_authority(SqliteAuthorityConfig::new(directory.path()))
        .await
        .unwrap();
    run_volume_lifecycle_revocation(sqlite.repository()).await;
    sqlite.integrity_check().await.unwrap();
}

async fn run_volume_lifecycle_revocation(repository: Arc<dyn AgentRegistryRepository>) {
    let approved = approve_initial_repository(repository.clone()).await;
    let request = RevokeVolumeForLifecycleRequest {
        tenant_id: approved.enrollment.tenant_id.clone(),
        storage_volume_id: approved.enrollment.storage_volume_id.clone(),
        deletion_id: DeletionId::new("deletion-volume-lifecycle").unwrap(),
        lifecycle_generation: LifecycleGeneration::new(2),
        request_digest: ContentDigest::hash(b"volume-lifecycle-revoke"),
        occurred_at_unix_ms: UnixMillis::new(500),
    };
    let first = repository
        .revoke_volume_for_lifecycle(request.clone())
        .await
        .unwrap();
    assert!(!first.replayed);
    assert_eq!(first.record.enrollment.state, AgentEnrollmentState::Revoked);
    assert_eq!(first.record.owner.state, VolumeOwnerState::Inactive);
    assert!(first.record.owner.active_agent_id.is_none());
    assert_eq!(
        first.record.mount.mount_generation.get(),
        approved.mount.mount_generation.get() + 1
    );
    assert_eq!(
        first.record.owner.owner_generation.get(),
        approved.owner.owner_generation.get() + 1
    );
    assert_eq!(
        first
            .record
    ...
    assert_eq!(
        first.record.volume_lifecycle_revocation.as_ref(),
        Some(&request.revocation())
    );
    let replay = repository
        .revoke_volume_for_lifecycle(request)
        .await
        .unwrap();
...
#[tokio::test]
async fn signed_bootstrap_replay_rejects_changed_order_or_unknown_fields() {
    let repository: Arc<dyn AgentRegistryRepository> = Arc::new(InMemoryAgentRegistry::new());
    let service = AgentRegistryService::new(repository, Arc::new(InMemoryClock::new(200)), 100);
    service
        .create_storage_enrollment_intent(rich_intent(
    ...
    assert!(changed_extension.signing_bytes().is_err());
...
fn resign_bootstrap_request(request: AgentBootstrapRequest) -> AgentBootstrapRequest {
    let public_key = request.proof.public_key_spki.as_public_key_bytes();
    let seed = (0..=u8::MAX)
        .find(|seed| test_key_pair(*seed).public_key().as_ref() == public_key.as_slice())
        .or_else(|| {
            public_key
                .iter()
                .all(|byte| *byte == public_key[0])
                .then_some(public_key[0])
        })
        .expect("bootstrap test request must use a deterministic test key");
    signed_bootstrap_request(request, &test_key_pair(seed))
}
...
async fn assert_replacement_transition_guards(repository: Arc<dyn AgentRegistryRepository>) {
    ...
    service
        ...
        .unwrap();
    let pending = service
        .bootstrap_agent_with_proof(replacement_bootstrap_request())
        .await
        .unwrap()
        .record;
...
async fn run_lifecycle(
    ...
    assert_eq!(
        service
            .bootstrap_agent_with_proof(resign_bootstrap_request(reused_identity))
            .await
            .unwrap_err()
            .code(),
        CentralErrorCode::AgentIdentityMismatch
...
fn independent_bootstrap_request() -> AgentBootstrapRequest {
    ...
    signed_bootstrap_request(request, &key_pair)
}

fn bootstrap_request(kind: AgentKind, bootstrap_token: &str) -> AgentBootstrapRequest {
    let key_pair = test_key_pair(match kind {
        AgentKind::Initial => 1,
...
fn workload_certificate_bundle(
    ...
    AgentWorkloadCertificateBundle {
        ...
        certificate_generation: neoengram_domain::protocol::CertificateGeneration::new(generation),
