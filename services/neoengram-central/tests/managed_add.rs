...
async fn assignment_id_conflict_leaves_job_queued_and_a_new_id_recovers() {
    ...
    let mut second_spec = first_spec.clone();
    second_spec.job_id = neoengram_domain::protocol::JobId::new("job-b").unwrap();
    second_spec.request_digest = second_spec.computed_request_digest().unwrap();
...
async fn accepted_assignment_retirement_prevents_stable_prefix_starvation() {
    ...
    let neoengram_domain::protocol::AssignmentOperation::Add { input, .. } = &pending[0].assignment
    else {
    ...
    let messages = control
        .deliverable_agent_messages(&first_target.agent_id, SessionGeneration::new(1), 1)
        .await
        .unwrap();
    assert_eq!(messages.len(), 1);
    let ControlMessage::Assignment(assignment) = &messages[0].body else {
        panic!("the next live assignment must not be starved");
    };
    let neoengram_domain::protocol::AssignmentOperation::Add { input, .. } = &assignment.assignment
    else {
...
#[tokio::test]
async fn create_rejects_unknown_operation_extensions() {
    let components = InMemoryComponents::new(100);
    let control = components.control_plane();
    let (actor, mut spec, _) = inputs(&components).await;
    spec.extensions
...
async fn metadata_staging_scopes_reused_batch_ids_by_tenant() {
    let stager = neoengram_central::InMemoryMetadataBatchStager::default();
...
struct ExpireLeaseOnObjectRead {
    inner: Arc<neoengram_central::InMemoryObjectCatalog>,
    clock: Arc<neoengram_central::InMemoryClock>,
    lease_expiry: u64,
    armed: AtomicBool,
}
...
impl ObjectCatalog for ExpireLeaseOnObjectRead {
    async fn record_placement(
        &self,
        evidence: &neoengram_central::ObjectPlacementEvidence,
    ) -> CentralResult<()> {
    ...
    async fn object_placement(
        ...
        object_id: ObjectId,
    ) -> CentralResult<Option<neoengram_central::ObjectPlacementEvidence>> {
        let placement = self
            .inner
            .object_placement(
                tenant_id,
    ...
    async fn artifact_placement_volumes(
        ...
        self.inner
            .artifact_placement_volumes(tenant_id, artifact_id)
            .await
    }
...
struct FailPublisherOnce {
    inner: Arc<neoengram_central::InMemoryIndexPublisher>,
    armed: AtomicBool,
}

impl FailPublisherOnce {
    fn new(inner: Arc<neoengram_central::InMemoryIndexPublisher>) -> Self {
        Self {
            inner,
            armed: AtomicBool::new(true),
        }
...
impl IndexPublisher for FailPublisherOnce {
    async fn initialize_snapshot(
        &self,
        request: neoengram_central::InitializeIndexSnapshotRequest,
...
impl AssignmentOutbox for FailRetireOnceOutbox {
    async fn reserve(
        &self,
        assignment: neoengram_domain::protocol::JobAssignment,
    ...
    ) -> CentralResult<neoengram_central::AssignmentRetireOutcome> {
        if self.armed.swap(false, Ordering::SeqCst) {
            return Err(CentralError::new(
                CentralErrorCode::StorageFailure,
    ...
    async fn pending_for_agent(
        ...
        agent_id: &AgentId,
        limit: usize,
    ) -> CentralResult<Vec<neoengram_domain::protocol::JobAssignment>> {
...
async fn build_scenario_with_candidate_overrides(
    ...
    // The fixture assignment freezes FastCDC; keep every manifest record aligned with that
    // layout so tests exercise publication validation rather than an unrelated layout mismatch.
    let chunking = ChunkingStrategy::FastCdc;
    let wire_chunking = WireChunkingStrategy::FastCdc;
