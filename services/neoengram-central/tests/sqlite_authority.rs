...
struct ContractFixture {
    ...
    reserved_assignment: neoengram_domain::protocol::JobAssignment,
...
async fn authority_contract_runs_against_in_memory_and_sqlite() {
    ...
    let unexpected_durability_count: i64 =
        sqlx::query_scalar("SELECT count(*) FROM durable_objects")
            ...
            .await
            .unwrap();
    let agent_registry_tables: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM sqlite_schema WHERE name = 'agent_registry_records'",
    )
    .fetch_one(&mut connection)
    .await
    .unwrap();
...
async fn sqlite_reopen_recovers_every_authority_port() {
    ...
    assert!(matches!(
        reopened
            ...
            .await
            .unwrap(),
        neoengram_central::AssignmentPublishOutcome::Published
    ));
...
async fn sqlite_rejects_wrong_application_id_and_user_version() {
    ...
    execute_raw(version_directory.path(), "PRAGMA user_version = 8").await;
...
async fn sqlite_concurrent_finalize_converges_from_a_publishing_candidate() {
    ...
    let control = Arc::new(ControlPlane::new(
        Arc::new(AllowAllAuthorizer),
        store,
        Arc::new(neoengram_central::InMemoryClock::new(600)),
    ));
...
async fn run_contract(store: AuthorityStore) -> ContractFixture {
    ...
    assert!(matches!(
        store
            ...
            .await
            .unwrap(),
        neoengram_central::AssignmentRetireOutcome::Retired
    ));
    assert!(matches!(
        store
            .outbox()
            ...
            .await
            .unwrap(),
        neoengram_central::AssignmentRetireOutcome::AlreadyRetired
    ));
    assert!(store
        .outbox()
        .pending_for_agent(&assigned_input.agent_id, 1)
    ...
    assert!(matches!(
        store
            ...
            .await
            .unwrap(),
        neoengram_central::AssignmentReserveOutcome::Reserved
    ));
    assert!(matches!(
        store
            .outbox()
            ...
            .await
            .unwrap(),
        neoengram_central::AssignmentReserveOutcome::Existing
    ));
...
async fn seed_pending_decision_contract(store: &AuthorityStore) -> JobKey {
    ...
    let messages = control
        .deliverable_agent_messages(&target_agent, SessionGeneration::new(1), 1)
        .await
        .unwrap();
    ...
    for tenant in ["tenant-a", "tenant-b"] {
        ...
        let event = AuditEvent {
            ...
            job_key: JobKey::new(
                ...
                neoengram_domain::protocol::JobId::new("shared-job").unwrap(),
            ),
...
fn job_spec(tenant: &str, job: &str) -> AddJobSpec {
    ...
    let mut spec = AddJobSpec {
        job_id: neoengram_domain::protocol::JobId::new(job).unwrap(),
        principal,
...
fn publication(
    ...
    IndexPublishRequest {
        job_key: JobKey::new(
            TenantId::new(tenant).unwrap(),
            neoengram_domain::protocol::JobId::new(job).unwrap(),
        ),
        index_key: key.clone(),
...
async fn assert_object_placement_scope(store: AuthorityStore) {
    ...
    let evidence = ObjectPlacementEvidence {
        receipt: ObjectReceiptRecord {
            ...
            tenant_id: TenantId::new("tenant-a").unwrap(),
            artifact_id: ArtifactId::new("artifact-a").unwrap(),
            job_id: neoengram_domain::protocol::JobId::new("job-placement-a").unwrap(),
            storage_volume_id: StorageVolumeId::new("volume-a").unwrap(),
            artifact_placement_id: ArtifactPlacementId::new("placement-a").unwrap(),
            object_id,
