...
pub fn diff_index_snapshots(
    ...
    type ContentIdentity = (neoengram_domain::core::ManifestId, u64, u64);
...
pub struct PreCommitRecord {
    ...
    /// Layout frozen when this Pre-commit starts and inherited by every Add/Manifest/Commit.
...
pub(crate) fn apply_job_sync(
    ...
    use neoengram_domain::protocol::{JobState, PublishDecision};
...
pub(crate) fn same_commit_request(
    ...
    left.key == right.key
        ...
        && left.data_layout == right.data_layout
        && left.commit.data_layout == right.commit.data_layout
...
pub(crate) fn validate_commit(commit: &CommitRecord) -> CentralResult<()> {
    ...
    let canonical_commit = canonical_commit_id_with_layout(canonical_core, commit.data_layout);
    if canonical_commit != commit.commit_id {
        return Err(protocol_invalid(
            "CommitId differs from root, parent, message, creation time, and data layout",
        ));
    }
    Ok(())
}

/// Computes the managed Commit identity in the new layout-aware domain. Both layouts carry an
/// explicit discriminator, so neither can collide with the layout-neutral core identity.
#[must_use]
pub fn canonical_commit_id_with_layout(core_id: CommitId, layout: CommitDataLayout) -> CommitId {
    let mut hasher = blake3::Hasher::new();
    hasher.update(b"neoengram-managed-commit-layout-v1\0");
    hasher.update(core_id.as_bytes());
    hasher.update(&[match layout {
        CommitDataLayout::FastCdc => 1,
        CommitDataLayout::WholeFile => 2,
    }]);
    CommitId::from_bytes(*hasher.finalize().as_bytes())
}
...
fn job_failure_notice(job: &JobRecord) -> PreCommitNotice {
    ...
    if let Some(neoengram_domain::protocol::PublishDecision::Reject { error, .. }) =
    ...
    let code = match job.state {
        neoengram_domain::protocol::JobState::Conflicted => "INDEX_CONFLICT",
        neoengram_domain::protocol::JobState::Cancelled => "PRECOMMIT_JOB_CANCELLED",
        neoengram_domain::protocol::JobState::TimedOut => "PRECOMMIT_JOB_TIMED_OUT",
        neoengram_domain::protocol::JobState::RecoveryRequired => "PRECOMMIT_JOB_RECOVERY_REQUIRED",
...
mod tests {
    ...
    #[test]
    fn commit_identity_always_binds_the_explicit_layout() {
        let core = CommitId::from_bytes([7; 32]);
        let fast_cdc = canonical_commit_id_with_layout(core, CommitDataLayout::FastCdc);
        let whole_file = canonical_commit_id_with_layout(core, CommitDataLayout::WholeFile);

        assert_ne!(fast_cdc, core);
        assert_ne!(whole_file, core);
        assert_ne!(fast_cdc, whole_file);
    }
}
