...
async fn discover_replicas(
    registry: &Arc<dyn GatewayRegistryRepository>,
) -> Result<Vec<GatewayReplicaRecord>, crate::CentralError> {
    let mut replicas = Vec::new();
    let mut pool_after = None;
    loop {
        let pools = registry
            .list_pools(&crate::GatewayPoolListRequest {
                ...
                state: Some(crate::GatewayPoolState::Ready),
                after: pool_after,
        ...
        for pool in pools {
            ...
            loop {
                ...
                if page_count < crate::GATEWAY_REGISTRY_MAX_PAGE_SIZE {
                    break;
                }
            }
        ...
        if pool_count < crate::GATEWAY_REGISTRY_MAX_PAGE_SIZE {
            break;
        }
    }
    Ok(replicas)
...
fn pool_snapshot_is_current_ready(
    observed: &crate::GatewayPoolRecord,
    current: Option<&crate::GatewayPoolRecord>,
) -> bool {
    current.is_some_and(|current| {
        current.state == crate::GatewayPoolState::Ready
...
impl ReplicaConnector {
    ...
    async fn connect_once(&self) -> Result<(), GatewayConnectorError> {
        ...
        handshake.timer(TokioTimer::new());
        handshake
...
mod tests {
    ...
    fn heartbeat_watchdog_only_refreshes_on_a_validated_replica_heartbeat() {
        use neoengram_domain::protocol::GatewayBackpressure;
        ...
        let heartbeat = GatewayControlMessage::ReplicaHeartbeat(
            neoengram_domain::protocol::GatewayReplicaHeartbeat {
                connected_agents: 0,
                active_streams: 0,
                queue_depth: 0,
            },
