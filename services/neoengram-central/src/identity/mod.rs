...
use neoengram_domain::protocol::{PrincipalId, PrincipalKind, PrincipalRef, TenantId};
...
impl OidcAuthenticator {
    ...
    pub async fn discover(config: OidcConfig) -> Result<Self, OidcInitializationError> {
        ...
        if config.allow_insecure_http && is_loopback_url(&config.issuer) {
...
fn is_public_method(context: &Context) -> bool {
    ...
    (service == "neoengram.system"
        ...
        || (service == "neoengram.s3.internal" && method == "authorize_s3_request")
}
...
pub enum Permission {
    ...
    SnapshotCreate,
    #[serde(rename = "s3.access.read")]
    S3AccessRead,
    #[serde(rename = "s3.access.manage")]
    S3AccessManage,
    #[serde(rename = "resource.lifecycle.read")]
    ResourceLifecycleRead,
    #[serde(rename = "resource.lifecycle.manage")]
    ResourceLifecycleManage,
...
impl Permission {
    ...
    pub const fn name(self) -> &'static str {
        match self {
            ...
            Self::S3AccessManage => "s3.access.manage",
            Self::ResourceLifecycleRead => "resource.lifecycle.read",
            Self::ResourceLifecycleManage => "resource.lifecycle.manage",
...
impl StaticRbacPolicy {
    ...
    pub fn authorize_global_identity(
        ...
        if self
            ...
            .is_some_and(|grant| {
                permission_is_granted(&grant.permissions, permission) && grant.tenants.contains("*")
            })
        {
            Ok(())
        } else {
    ...
    pub fn is_allowed(
        ...
        self.grants.get(principal.id.as_str()).is_some_and(|grant| {
            permission_is_granted(&grant.permissions, permission)
    ...
    pub fn tenant_visibility(
        ...
        if !permission_is_granted(&grant.permissions, permission) {
            return TenantVisibility::None;
        }
    ...
    pub fn permission_names(&self, principal: &PrincipalRef, tenant_id: &TenantId) -> Vec<String> {
        ...
        if permissions.contains(&Permission::TenantAdmin) {
            permissions.extend([
...
fn permission_is_granted(permissions: &BTreeSet<Permission>, requested: Permission) -> bool {
    permissions.contains(&requested)
        || (permissions.contains(&Permission::TenantAdmin)
            && matches!(
                requested,
                Permission::S3AccessRead
                    | Permission::S3AccessManage
...
mod tests {
    ...
    #[test]
    fn permission_deserialization_rejects_removed_aliases() {
        for legacy in ["create_add_job", "query_job", "finalize_add"] {
            assert!(serde_json::from_str::<Permission>(&format!("\"{legacy}\"")).is_err());
        }
        assert_eq!(
    ...
    #[test]
    fn tenant_admin_inherits_resource_management_only_inside_its_tenant_scope() {
        let policy = StaticRbacPolicy::from_json(
            br#"{
                "roles": {"tenant-admin": {"permissions": ["tenant.admin"]}},
                "bindings": [{
                    "principal_id": "user-a",
                    "roles": ["tenant-admin"],
                    "tenants": ["tenant-a"]
        ...
        assert!(policy.is_allowed(identity().principal(), Permission::S3AccessRead, &tenant_a));
        assert!(policy.is_allowed(
            identity().principal(),
            Permission::S3AccessManage,
            &tenant_a
        ...
        assert!(!policy.is_allowed(identity().principal(), Permission::S3AccessRead, &tenant_b));
        assert!(!policy.is_allowed(
            identity().principal(),
            Permission::ResourceLifecycleManage,
            &tenant_b
        ));
        assert!(!policy.is_allowed(identity().principal(), Permission::StorageCreate, &tenant_a));
        assert_eq!(
            policy.permission_names(identity().principal(), &tenant_a),
            vec![
                "tenant.admin".to_owned(),
                "s3.access.read".to_owned(),
                "s3.access.manage".to_owned(),
    ...
    fn disabled_rbac_binding_never_grants_permissions() {
        let policy = StaticRbacPolicy::from_json(
            br#"{
                "roles": {"reader": {"permissions": ["job.read"]}},
                "bindings": [{
                    "principal_id": "user-a",
                    "roles": ["reader"],
                    "tenants": ["tenant-a"],
