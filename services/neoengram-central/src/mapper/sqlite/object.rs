...
use neoengram_domain::protocol::{ArtifactId, PlacementGeneration, StorageVolumeId, TenantId};
...
impl ObjectCatalog for SqliteAuthorityStore {
    ...
    async fn artifact_placement_volumes(
        ...
        values
            .into_iter()
            .map(|value| {
                StorageVolumeId::new(value).map_err(|error| {
                    storage_corruption(format!(
                        "stored object placement Volume ID is invalid: {error}"
                    ))
                })
            })
            .collect()
    ...
    async fn volume_unique_artifact_replicas(
        ...
        values
            .into_iter()
            .map(|value| {
                ArtifactId::new(value).map_err(|error| {
                    storage_corruption(format!(
                        "stored object placement Artifact ID is invalid: {error}"
                    ))
                })
            })
            .collect()
