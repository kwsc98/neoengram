...
use neoengram_domain::core::{ContentDigest, Manifest, ManifestId};
...
use crate::{
    ...
    mapper::sqlite::agent_registry::{
        open_sqlite_agent_registry_with_datasource, SqliteAgentRegistry,
    },
...
impl std::fmt::Debug for SqliteAuthorityConfig {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("SqliteAuthorityConfig")
            .field("path", &"[REDACTED]")
...
impl SqliteAuthority {
    #[must_use]
    pub fn repository(&self) -> Arc<dyn crate::AgentRegistryRepository> {
        self.authority_store()
            .agent_registry()
            .expect("SQLite authority must expose its Agent registry")
    }
    ...
    pub fn gateway_repository(&self) -> Arc<dyn crate::GatewayRegistryRepository> {
        self.authority_store()
            .gateway_registry()
            .expect("SQLite authority must expose its Gateway registry")
    }
