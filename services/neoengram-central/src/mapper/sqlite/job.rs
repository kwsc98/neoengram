...
use neoengram_domain::protocol::{AgentId, TenantId};
...
impl JobRepository for SqliteAuthorityStore {
    ...
    async fn list_recoverable(
        ...
        for row in rows {
            ...
            let job_id = neoengram_domain::protocol::JobId::new(
...
fn validate_job_identity(job: &JobRecord) -> CentralResult<()> {
    job.spec
        .job_id
        .as_str()
        .parse::<neoengram_domain::protocol::JobId>()?;
...
pub(super) const fn job_state_name(state: neoengram_domain::protocol::JobState) -> &'static str {
