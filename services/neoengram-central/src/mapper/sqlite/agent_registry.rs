...
use crate::{
    ...
    datasource::sqlite::authority::SqliteAuthorityDataSource, ensure_immutable_registry_scope,
    revoke_for_volume_lifecycle, transition_storage_enrollment, validate_registry_insert,
    ...
    CentralResult, PvcVolumeBinding, RevokeVolumeForLifecycleOutcome,
    RevokeVolumeForLifecycleRequest, StorageEnrollmentRegistrationKind, StorageEnrollmentState,
    AGENT_ENROLLMENT_MAX_PAGE_SIZE, AGENT_ENROLLMENT_MAX_QUERY_CHARS,
};
...
pub(crate) async fn open_sqlite_agent_registry_with_datasource(
...
pub(super) struct SqliteAgentRegistryStore {
    ...
    pub(super) datasource: Arc<SqliteAuthorityDataSource>,
}
...
impl AgentRegistryRepository for SqliteAgentRegistryStore {
    ...
    async fn revoke_volume_for_lifecycle(
    ...
    ) -> CentralResult<RevokeVolumeForLifecycleOutcome> {
        let mut transaction = self.pool.begin().await.map_err(storage_error)?;
        let rows = sqlx::query(&format!(
            "SELECT {AGENT_RECORD_COLUMNS} FROM agent_registry_records \
             WHERE tenant_id = ? AND storage_volume_id = ?"
        ))
        .bind(request.tenant_id.as_str())
        .bind(request.storage_volume_id.as_str())
        .fetch_all(&mut *transaction)
        .await
        .map_err(storage_error)?;
        let expected_revocation = request.revocation();
        let mut current = None;
        for row in rows {
            let record = decode_row(row)?;
            if let Some(existing) = &record.volume_lifecycle_revocation {
                if existing == &expected_revocation {
                    transaction.commit().await.map_err(storage_error)?;
                    return Ok(RevokeVolumeForLifecycleOutcome {
                        record,
                        replayed: true,
                    });
                }
                if existing.deletion_id == request.deletion_id {
                    return Err(CentralError::new(
                        CentralErrorCode::ConcurrentUpdate,
                        "Volume lifecycle revocation identity is bound to another request",
                    )
                    .with_retryable(false));
                }
            }
            if record.enrollment.state == AgentEnrollmentState::Approved
                && current.replace(record).is_some()
            {
                return Err(storage_corruption(
                    "multiple approved Agents own one Tenant Volume",
                ));
        ...
        let current = current.ok_or_else(|| {
            CentralError::new(
                ...
                "StorageVolume has no approved Agent enrollment to revoke",
            )
            .with_retryable(false)
        })?;
        let previous_resource_version = current.resource_version.get();
        let revoked = revoke_for_volume_lifecycle(current, &request)?;
        let result = sqlx::query(
            "UPDATE agent_registry_records SET bootstrap_request_id = ?, decision_request_id = ?, \
             installation_id = ?, public_key_fingerprint = ?, pvc_binding_role = NULL, \
             enrollment_state = ?, registration_kind = ?, \
             enrollment_created_at_unix_ms = ?, display_name = ?, \
             resource_version = ?, payload = ? \
             WHERE enrollment_id = ? AND agent_id = ? AND resource_version = ?",
        ...
        if result.rows_affected() != 1 {
            return Err(CentralError::new(
                CentralErrorCode::ConcurrentUpdate,
                "Agent registry ResourceVersion changed during Volume lifecycle revocation",
            ));
        }
        transaction.commit().await.map_err(storage_error)?;
        Ok(RevokeVolumeForLifecycleOutcome {
            record: revoked,
            replayed: false,
        })
    }
