...
pub(crate) async fn validate_staged_metadata(
    ...
    let expected_chunking: ChunkingStrategy = assignment.data_layout.into();
    if manifest_fragments
        .values()
        .any(|manifest| manifest.chunking != expected_chunking)
    {
        return Err(invalid(
            CentralErrorCode::MetadataInvalid,
            "Manifest chunking differs from the data layout frozen in the Add assignment",
        ));
    }
...
fn index_delta_path(mutation: &IndexDeltaRecord) -> &neoengram_domain::core::LogicalPath {
    match mutation {
...
mod tests {
    ...
    use neoengram_domain::core::{
        ChunkRef, ChunkingStrategy, LogicalPath, Manifest, ManifestId, ObjectId,
    };
    use neoengram_domain::protocol::{
