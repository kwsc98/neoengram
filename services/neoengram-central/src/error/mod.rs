...
pub fn map_central_error(error: CentralError) -> Error {
    ...
    let (category, status, public_message, retryable, retry_after_ms) = match code {
        ...
        CentralErrorCode::SnapshotDeliveryUnavailable => (
            ErrorCategory::Unavailable,
            Some(StatusCode::SERVICE_UNAVAILABLE),
            "no ready Agent can provide the requested SnapshotDelivery mode",
            true,
            Some(1_000),
        ),
        CentralErrorCode::SnapshotDeliveryUnsupported => (
            ErrorCategory::Conflict,
            None,
            "the current Agent does not support the requested SnapshotDelivery mode",
            false,
            None,
        ),
...
fn conflict_message(code: CentralErrorCode) -> &'static str {
    match code {
        ...
        CentralErrorCode::SnapshotDeliveryUnavailable => {
            "no ready Agent is available for this SnapshotDelivery"
        }
        CentralErrorCode::SnapshotDeliveryUnsupported => {
