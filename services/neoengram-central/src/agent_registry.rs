...
use neoengram_domain::protocol::{
    ...
    AgentMountStatusReport, AgentSignatureAlgorithm, AgentWorkloadCertificateBundle, DeletionId,
    Ed25519PublicKeySpki, Ed25519Signature, EdgeClusterId, Extensions, LifecycleGeneration,
...
/// Capability required for every task-fenced v2 operation and data-plane report.
pub const AGENT_CAPABILITY_OPERATION_TASK_V2: &str = "operation_task_v2";
...
/// Stable identity for a StorageVolume lifecycle revocation.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct VolumeLifecycleRevocation {
...
pub struct RevokeVolumeForLifecycleRequest {
    pub tenant_id: TenantId,
...
impl RevokeVolumeForLifecycleRequest {
    #[must_use]
    pub fn revocation(&self) -> VolumeLifecycleRevocation {
        VolumeLifecycleRevocation {
...
pub struct RevokeVolumeForLifecycleOutcome {
...
pub struct AgentRegistryRecord {
    ...
    /// Present only when Volume lifecycle finalization, rather than Agent replacement, revoked
    /// this enrollment. The identity makes finalization retries safe across process restarts.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub volume_lifecycle_revocation: Option<VolumeLifecycleRevocation>,
}
...
impl AgentRegistryService {
    ...
    pub async fn bootstrap_agent_with_proof(
        ...
        self.bootstrap_agent_inner(request, credential_evidence, false)
            .await
    }
    ...
    pub async fn bootstrap_storage_enrollment(
        ...
        self.bootstrap_agent_inner(request.request, credential_evidence, true)
            .await
    }
    ...
    pub async fn bootstrap_status_with_clock_skew(
        ...
        let evidence = candidate
            .credential_evidence
            .as_ref()
            .ok_or_else(current_record_corruption)?;
    ...
    async fn bootstrap_agent_inner(
        &self,
        request: AgentBootstrapRequest,
        credential_evidence: VerifiedAgentCredentialEvidence,
        ...
        let candidate = AgentCandidateRecord {
            ...
            probe: request.probe,
            credential_evidence: Some(credential_evidence.evidence),
            proof_of_possession_status: Some(AgentProofOfPossessionStatus::Verified),
        };
        if let Some(existing) = self
            .repository
    ...
    /// Returns the complete current owner only while the Volume is Ready. S3 ticket issuance uses
    /// this single observation to bind Agent, mount, owner and session generations; callers must
    /// still fence it against the Gateway RouteLease before issuing a ticket.
    pub async fn current_ready_volume_record(
        &self,
        tenant_id: &TenantId,
        storage_volume_id: &StorageVolumeId,
    ) -> CentralResult<Option<AgentRegistryRecord>> {
        ...
        if record.derived_volume_state(self.clock.now(), self.heartbeat_timeout_ms)
            != DerivedVolumeState::Ready
        {
            return Ok(None);
        }
    ...
    pub async fn current_ready_volume_supports_materialization(
        ...
        Ok(record.instance.is_some_and(|instance| {
            instance
                ...
                && instance
                    .capabilities
                    .contains(AGENT_CAPABILITY_OPERATION_TASK_V2)
        }))
    }
...
fn require_current_registry_record(record: &AgentRegistryRecord) -> CentralResult<()> {
    if record.storage_enrollment.record_format == AgentRegistryRecordFormat::CurrentV3 {
        ...
        Err(registry_corruption(
            "Agent registry record format is not current",
        ))
    }
}
...
fn require_public_enrollment_metadata(record: &AgentRegistryRecord) -> CentralResult<()> {
        ...
        && has_verified_candidate_evidence
    {
        ...
        Err(registry_corruption(
            "Agent enrollment metadata is incomplete for the current authority format",
        ))
    }
}
...
fn current_record_corruption() -> CentralError {
    registry_corruption("Agent registry record is not current")
}
...
fn active_instance(agent_id: &AgentId, candidate: AgentCandidateRecord) -> AgentInstanceRecord {
    AgentInstanceRecord {
        ...
        wire_version: candidate.wire_version,
...
fn derive_session_id(
    ...
    let digest = neoengram_domain::protocol::jcs_blake3(&SessionIdentity {
        agent_id,
        boot_id,
        session_generation,
    })?;
...
pub(crate) fn revoke_for_volume_lifecycle(
    mut record: AgentRegistryRecord,
    request: &RevokeVolumeForLifecycleRequest,
) -> CentralResult<AgentRegistryRecord> {
    if record.enrollment.tenant_id != request.tenant_id
        || record.enrollment.storage_volume_id != request.storage_volume_id
    {
        return Err(error(
            CentralErrorCode::AgentIdentityMismatch,
            "Volume lifecycle revocation scope differs from the Agent enrollment",
        ));
    }
    if request.lifecycle_generation.get() == 0 {
        return Err(error(
            CentralErrorCode::ProtocolInvalid,
            "Volume lifecycle generation must be non-zero",
        ));
    }
    if record.volume_lifecycle_revocation.is_some()
        || record.enrollment.state != AgentEnrollmentState::Approved
        || record.owner.state == VolumeOwnerState::Inactive
    {
        return Err(error(
            CentralErrorCode::InvalidState,
            "Volume lifecycle revocation requires the current approved Agent owner",
        ));
    }

    let next_mount_generation = record
        .mount
        .mount_generation
        .get()
        .checked_add(1)
        .ok_or_else(|| {
            error(
                CentralErrorCode::Internal,
                "Agent mount generation is exhausted",
            )
        })?;
    let next_owner_generation = record
        .owner
        .owner_generation
        .get()
        .checked_add(1)
        .ok_or_else(|| {
            error(
                CentralErrorCode::Internal,
                "Volume owner generation is exhausted",
            )
        })?;
    let instance = record.instance.as_ref().ok_or_else(|| {
        error(
            CentralErrorCode::AgentIdentityMismatch,
            "approved Volume owner has no Agent instance",
        )
    })?;
    let next_session_generation = instance
        .session_generation
        .map_or(Some(1), |generation| generation.get().checked_add(1))
        .ok_or_else(|| {
            error(
                CentralErrorCode::Internal,
                "Agent session generation is exhausted",
            )
        })?;
    ...
    let instance = record
        .instance
        .as_mut()
        .expect("approved Volume owner instance was checked above");
    instance.state = AgentInstanceState::Revoked;
    instance.session_generation = Some(SessionGeneration::new(next_session_generation));
    instance.active_boot_id = None;
    instance.active_session_id = None;
    instance.session_open_expected_resource_version = None;
    instance.session_opened_at_unix_ms = None;
    ...
    record.owner.active_agent_mount_id = None;
    record.owner.state = VolumeOwnerState::Inactive;
    record.volume_lifecycle_revocation = Some(request.revocation());
    transition_storage_enrollment(
        &mut record,
        None,
        AgentEnrollmentLifecycleAuditKind::Revoked,
...
pub(crate) fn ensure_immutable_registry_scope(
    ...
    let enrollment_scope_matches = stored.enrollment.token_id == updated.enrollment.token_id
        ...
        && stored.volume_lifecycle_revocation == updated.volume_lifecycle_revocation;
...
pub(crate) fn validate_registry_replacement_transition(
    ...
    let lifecycle_is_atomic = revoked.storage_enrollment.state.is_none()
...
pub(crate) fn validate_registry_record(record: &AgentRegistryRecord) -> CentralResult<()> {
    ...
    let replacement_revocation = record.enrollment.replaced_by_enrollment_id.is_some();
    let volume_lifecycle_revocation = record.volume_lifecycle_revocation.is_some();
    if record.enrollment.state == AgentEnrollmentState::Revoked
        && replacement_revocation == volume_lifecycle_revocation
    {
        return Err(registry_corruption(
            "a revoked enrollment must bind exactly one replacement or Volume lifecycle operation",
        ));
    }
    if record.enrollment.state != AgentEnrollmentState::Revoked && volume_lifecycle_revocation {
        return Err(registry_corruption(
            "only a revoked enrollment can bind a Volume lifecycle operation",
        ));
    }
    Ok(())
}
