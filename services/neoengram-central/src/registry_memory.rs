...
use crate::{
    ...
    RevokeVolumeForLifecycleOutcome, RevokeVolumeForLifecycleRequest, StorageEnrollmentState,
...
impl AgentRegistryRepository for InMemoryAgentRegistry {
    ...
    async fn expire_stale_token_intents(
        ...
        for record in records.values() {
                ...
                && record.enrollment.expires_at_unix_ms.get() <= now_unix_ms.get()
            {
                ...
                expired.resource_version = neoengram_domain::protocol::ResourceVersion::new(next);
    ...
    async fn revoke_volume_for_lifecycle(
        ...
        for (enrollment_id, record) in records.iter() {
            ...
            if let Some(existing) = &record.volume_lifecycle_revocation {
                if existing == &request.revocation() {
            ...
            if record.enrollment.state == AgentEnrollmentState::Approved
                && current_id.replace(enrollment_id.clone()).is_some()
            {
                return Err(CentralError::new(
                    CentralErrorCode::StorageFailure,
                    "multiple approved Agents own one Tenant Volume",
        ...
        let current = records
            .get(&current_id)
            .expect("current enrollment remains locked")
            .clone();
        let revoked = crate::revoke_for_volume_lifecycle(current, &request)?;
        records.insert(current_id, revoked.clone());
