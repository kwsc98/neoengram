...
use http::StatusCode;
use neoengram_domain::core::{CommitId, FileRecord, Manifest, ManifestId};
use neoengram_domain::protocol::{
    ...
    JobState, MountGeneration, OwnerGeneration, RequestId, S3ReadTicket, SessionGeneration,
    SessionId, StorageVolumeId, WireChunkRef, CURRENT_WIRE_VERSION,
};

use crate::agent_transport::{AgentDataPlaneHandler, AgentHttpError};

/// Authoritative metadata implementation behind the Gateway-routed Agent index action.
...
impl AgentDataPlaneService {
    ...
    async fn query_index_page(
        ...
        payload.validate()?;
        if let Some(ticket) = payload.s3_ticket.clone() {
            return self
                .query_s3_index_page(request_id, context, ticket, payload)
                .await;
        }
        let job = self
        ...
        let (version, records) = match job.operation {
            ...
            JobOperation::SnapshotDelivery => {
                self.load_snapshot_delivery_index(&job, &payload).await?
            }
        };
    ...
    async fn query_s3_index_page(
        ...
        validate_s3_ticket_context(&ticket, context, &payload.artifact_id)?;
        if ticket.index_digest != payload.index_version.digest
            || ticket.artifact_id != payload.artifact_id.as_str()
            || ticket.snapshot_id
                != payload
                    .snapshot_id
                    .as_ref()
                    ...
                    .unwrap_or_default()
        {
            return Err(scope_mismatch(
                "S3 Index query does not match the immutable ticket",
            ));
        }
        let commit = self.load_s3_commit(&ticket).await?;
        if commit.index_version.digest != ticket.index_digest {
            return Err(authority_integrity_error(
                "S3 ticket Index digest differs from the authoritative Commit",
            ));
        }
        let (page_count, records) = page_index_records(
            &commit.records,
    ...
    async fn query_s3_manifest_page(
        &self,
        request_id: RequestId,
        context: &AgentRequestContext,
        ticket: S3ReadTicket,
        ...
        if !commit
            .records
            .iter()
            .any(|record| record.manifest_id == payload.manifest_id)
        {
            return Err(scope_mismatch(
                "S3 Manifest is not referenced by the frozen Commit",
            ));
        }
        let manifest = self
            .authority
            .manifest(
                &neoengram_domain::protocol::TenantId::new(ticket.tenant_id.clone()).map_err(
                    |_| {
                        CentralError::new(
                            CentralErrorCode::ProtocolInvalid,
                            "S3 ticket Tenant ID is invalid",
                        )
                    },
                )?,
                &payload.artifact_id,
            ...
            .await?
            .ok_or_else(|| {
                authority_integrity_error("frozen Commit references a missing Manifest")
            })?;
        ...
        Ok(AgentManifestPageQueryResponse {
            wire_version: CURRENT_WIRE_VERSION,
            request_id,
    ...
    async fn load_s3_commit(&self, ticket: &S3ReadTicket) -> CentralResult<crate::CommitRecord> {
        let tenant_id = neoengram_domain::protocol::TenantId::new(ticket.tenant_id.clone())
            .map_err(|_| {
                CentralError::new(
                    CentralErrorCode::ProtocolInvalid,
                    "S3 ticket Tenant ID is invalid",
        ...
        let project_id = neoengram_domain::protocol::ProjectId::new(ticket.project_id.clone())
            .map_err(|_| {
                CentralError::new(
                    CentralErrorCode::ProtocolInvalid,
                    "S3 ticket Project ID is invalid",
                )
            })?;
        let artifact_id = neoengram_domain::protocol::ArtifactId::new(ticket.artifact_id.clone())
            .map_err(|_| {
            CentralError::new(
                CentralErrorCode::ProtocolInvalid,
        ...
        precommits
            .get_commit(
                ...
                &project_id,
                &artifact_id,
                CommitId::from_digest(ticket.commit_id),
            )
            .await?
            .ok_or_else(|| authority_integrity_error("S3 ticket Commit is missing from authority"))
    }
    ...
    async fn load_snapshot_delivery_index(
        &self,
        job: &JobRecord,
        payload: &AgentIndexPageQueryPayload,
    ) -> CentralResult<(
        ...
        let spec = snapshot_delivery_spec(job)?;
        let snapshot_id = payload
            .snapshot_id
            .as_ref()
            .ok_or_else(|| scope_mismatch("SnapshotDelivery Index query requires snapshot_id"))?;
        if spec.artifact_id != payload.artifact_id || &spec.snapshot_id != snapshot_id {
            return Err(scope_mismatch(
                "Index query scope differs from the persisted SnapshotDelivery assignment",
            ));
        }
        let commit = self.load_snapshot_delivery_commit(job).await?;
        if commit.index_version.digest != spec.source_index_digest
            || commit.index_version.digest != payload.index_version.digest
        {
            return Err(CentralError::new(
                CentralErrorCode::ConcurrentUpdate,
                "SnapshotDelivery Commit IndexVersion differs from the frozen assignment",
            ));
        }
        Ok((commit.index_version, commit.records))
    }
    ...
    async fn query_manifest_page(
        ...
        if let Some(ticket) = payload.s3_ticket.clone() {
            return self
                .query_s3_manifest_page(request_id, context, ticket, payload)
                .await;
        }
        let job = self
            .load_owned_job(&payload.tenant_id, &payload.job_id, context)
        ...
        let commit = match job.operation {
            ...
            JobOperation::SnapshotDelivery => {
                ...
                if spec.artifact_id != payload.artifact_id {
                    return Err(scope_mismatch(
                        "Manifest query Artifact differs from the SnapshotDelivery assignment",
                    ));
                }
                self.load_snapshot_delivery_commit(&job).await?
            }
            JobOperation::Add => {
                return Err(scope_mismatch(
    ...
    async fn load_materialization_commit(
        &self,
        job: &JobRecord,
    ) -> CentralResult<crate::CommitRecord> {
        let spec = workspace_spec(job)?;
    ...
    async fn load_snapshot_delivery_commit(
        &self,
        job: &JobRecord,
    ) -> CentralResult<crate::CommitRecord> {
        let spec = snapshot_delivery_spec(job)?;
        ...
        precommits
            ...
            .await?
            .ok_or_else(|| {
                authority_integrity_error("SnapshotDelivery Commit is missing from authority")
            })
    }
    ...
    match job.operation {
        ...
        JobOperation::SnapshotDelivery => {
            if job.assignment.is_some() || job.workspace_assignment.is_some() {
                return Err(authority_integrity_error(
                    "SnapshotDelivery Job contains another operation's assignment",
                ));
            }
            let spec = snapshot_delivery_spec(job)?;
            let assignment = job.delivery_assignment.as_ref().ok_or_else(|| {
                CentralError::new(
                    CentralErrorCode::InvalidState,
            ...
            {
                return Err(authority_integrity_error(
                    "SnapshotDelivery assignment differs from its immutable Job scope",
                ));
            }
            Ok(ExecutionPlacement {
                agent_id: assignment.agent_id.clone(),
...
fn workspace_spec(job: &JobRecord) -> CentralResult<&crate::WorkspaceMaterializeSpec> {
...
fn snapshot_delivery_spec(job: &JobRecord) -> CentralResult<&crate::SnapshotDeliverySpec> {
    job.delivery_spec.as_ref().ok_or_else(|| {
        authority_integrity_error("SnapshotDelivery Job lost its immutable operation scope")
    })
}
...
fn validate_s3_ticket_context(
    ticket: &S3ReadTicket,
    context: &AgentRequestContext,
    artifact_id: &neoengram_domain::protocol::ArtifactId,
) -> CentralResult<()> {
    if ticket.agent_id != context.agent_id
        || ticket.session_generation != context.session_generation
        || ticket.artifact_id != artifact_id.as_str()
    {
        return Err(scope_mismatch(
            "S3 ticket is not bound to this Agent session",
        ));
    }
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use neoengram_domain::core::{ChunkRef, ChunkingStrategy, LogicalPath, ObjectId};
    ...
    fn manifest_must_be_referenced_by_and_match_the_frozen_commit_index() {
        ...
        let unreferenced = require_manifest_reference(
            &records,
            ManifestId::from_digest(neoengram_domain::core::ContentDigest::hash(b"other")),
        )
        .unwrap_err();
