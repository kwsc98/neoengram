...
pub(crate) fn validate_gateway_replica(record: &GatewayReplicaRecord) -> CentralResult<()> {
    ...
    if !record
        ...
        || record.supported_protocol_versions.len() > 128
        || record.capabilities.len() > 128
    {
        return invalid("GatewayReplica must support protocol v1 within capability limits");
    }
...
pub(crate) fn validate_gateway_replica_parent_replace(
    ...
    let immutable_runtime_fields_match = stored.control_endpoint == next.control_endpoint
        ...
        && stored.supported_protocol_versions == next.supported_protocol_versions
...
fn validate_gateway_credential(credential: &GatewayReplicaCredential) -> CentralResult<()> {
    ...
    // Active records written before recoverable delivery was introduced may not retain the
    // public certificate bytes. They remain valid, but only newly prepared activations can be
    // resumed because prepare always persists `certificate`.
    let issued = metadata_issued;
