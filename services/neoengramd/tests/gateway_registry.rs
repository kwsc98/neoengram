...
use neoengramd::{
    open_sqlite_agent_registry, open_sqlite_authority, AcquireAgentRouteLeaseRequest,
    AcquireAgentSessionRouteRequest, AgentRegistryRepository, AgentRegistryService,
    AgentRouteLease, AgentRouteLeaseListRequest, CentralErrorCode, CloseAgentSessionRequest,
    GatewayCredentialState, GatewayInsertOutcome, GatewayPoolListRequest, GatewayPoolRecord,
    GatewayPoolState, GatewayRegistryRepository, GatewayReplicaCertificateRecord,
    GatewayReplicaCredential, GatewayReplicaListRequest, GatewayReplicaRecord, GatewayReplicaState,
    InMemoryClock, InMemoryComponents, InMemoryGatewayRegistry, OpenAgentSessionRequest,
    ReleaseAgentRouteLeaseRequest, RenewAgentRouteLeaseRequest, SqliteAgentRegistryConfig,
    SqliteAuthorityConfig, AGENT_ROUTE_LEASE_MAX_TTL_MS,
};
...
#[tokio::test]
async fn sqlite_agent_registry_migrates_v6_to_v7_atomically() {
    let directory = TempDir::new().unwrap();
    drop(
    ...
    execute_raw(
        directory.path(),
        "DROP TABLE agent_route_leases;
         DROP TABLE gateway_replica_credentials;
         DROP TABLE gateway_replica_records;
         DROP TABLE gateway_pool_records;
         DROP INDEX gateway_agent_cluster_identity;
         PRAGMA user_version = 6;",
    )
    .await;

    let migrated = open_sqlite_agent_registry(SqliteAgentRegistryConfig::new(directory.path()))
        .await
        .unwrap();
    assert!(migrated
        .gateway_repository()
    ...
    let version = registry_user_version(directory.path()).await;
    assert_eq!(version, 7);
}

#[tokio::test]
async fn sqlite_v6_to_v7_rejects_partially_present_gateway_schema() {
    let directory = TempDir::new().unwrap();
    drop(
        open_sqlite_agent_registry(SqliteAgentRegistryConfig::new(directory.path()))
            .await
    ...
    execute_raw(
        directory.path(),
        "DROP INDEX gateway_replica_credential_state;
         PRAGMA user_version = 6;",
    )
    .await;
    ...
    let error = open_sqlite_agent_registry(SqliteAgentRegistryConfig::new(directory.path()))
        .await
        .err()
        .expect("partial Gateway schema must fail closed");
    assert_eq!(error.code(), CentralErrorCode::StorageFailure);
    assert_eq!(registry_user_version(directory.path()).await, 6);
}

#[tokio::test]
async fn sqlite_v6_to_v7_rolls_back_when_full_schema_validation_fails() {
    let directory = TempDir::new().unwrap();
    drop(
        open_sqlite_agent_registry(SqliteAgentRegistryConfig::new(directory.path()))
            .await
    ...
    execute_raw(
        ...
        "DROP TABLE agent_route_leases;
         ...
         DROP TABLE gateway_pool_records;
         DROP INDEX gateway_agent_cluster_identity;
         CREATE TABLE unexpected_v6_object (id INTEGER PRIMARY KEY) STRICT;
         PRAGMA user_version = 6;",
    )
    .await;
    ...
    let error = open_sqlite_agent_registry(SqliteAgentRegistryConfig::new(directory.path()))
        .await
        .err()
        .expect("an extra v6 schema object must fail migration");
    assert_eq!(error.code(), CentralErrorCode::StorageFailure);
    assert_eq!(registry_user_version(directory.path()).await, 6);
    let objects = registry_schema_object_names(directory.path()).await;
    assert!(objects.contains("unexpected_v6_object"));
    assert!(objects
        .iter()
        .all(|name| !name.starts_with("gateway_") && !name.starts_with("agent_route_")));
}
...
async fn approve_agent(
    ...
    service
        ...
        .unwrap();
    let pending = service
        .bootstrap_agent(agent_bootstrap_request())
        .await
        .unwrap();
    service
        .decide_enrollment(
...
fn agent_bootstrap_request() -> AgentBootstrapRequest {
    let proof = AgentBootstrapProof::new(
        ...
        Ed25519Signature::from_bytes([7; 64]),
    );
...
async fn registry_user_version(root: &Path) -> i64 {
    let options = SqliteConnectOptions::new().filename(root.join("agent-registry.sqlite3"));
    let mut connection = SqliteConnection::connect_with(&options).await.unwrap();
...
async fn registry_schema_object_names(root: &Path) -> BTreeSet<String> {
    let options = SqliteConnectOptions::new().filename(root.join("agent-registry.sqlite3"));
    let mut connection = SqliteConnection::connect_with(&options).await.unwrap();
