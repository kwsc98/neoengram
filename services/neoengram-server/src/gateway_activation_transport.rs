...
use neoengram_protocol::{
    ...
    GATEWAY_REPLICA_BOOTSTRAP_CERTIFICATE_PATH, GATEWAY_REPLICA_BOOTSTRAP_CHALLENGE_PATH,
    MAX_GATEWAY_CONTROL_FRAME_BYTES, PROTOCOL_VERSION_V1,
};
use neoengramd::GatewayReplicaRecord;
...
mod tests {
    ...
    use neoengram_protocol::{
        ...
        GatewayReplicaId, Generation, PrincipalId, PrincipalKind, PrincipalRef, ProtocolVersion,
        ResourceVersion, UnixMillis,
    };
    use neoengramd::{
    ...
    async fn client_retries_delivery_from_persisted_prepare_without_reissuing() {
        ...
        let pending = repository
            ...
            .unwrap()
            .unwrap();
        assert_eq!(pending.state, neoengramd::GatewayReplicaState::Pending);
        assert_eq!(
            pending.credential.state,
            neoengramd::GatewayCredentialState::PendingCertificateDelivery
        );
        ...
        assert_eq!(
            second.replica.state,
            neoengramd::GatewayReplicaState::Active
        );
    ...
    async fn concurrent_clients_have_one_commit_winner() {
        ...
        let stored = repository
            ...
            .unwrap()
            .unwrap();
        assert_eq!(stored.state, neoengramd::GatewayReplicaState::Active);
    }
}
