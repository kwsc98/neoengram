...
impl GatewayRegistryService {
    ...
    pub async fn create_pool(
        ...
        let (record, replayed) = match self.repository.insert_pool(record).await {
            ...
            Err(error) if error.code() == neoengramd::CentralErrorCode::GatewayIdentityConflict => {
    ...
    pub async fn update_pool(
        ...
        if record.resource_version != expected {
            return Err(map_central_error(neoengramd::CentralError::new(
    ...
    pub async fn create_replica(
        ...
        if capabilities.len() != request.capabilities.len()
            || protocol_versions.len() != request.supported_protocol_versions.len()
        {
            return Err(invalid_request(
                "GatewayReplica capabilities and protocol versions must be unique",
            ));
        }
        ...
        let (record, activation_token, replayed) =
            match self.repository.insert_replica(record).await {
                ...
                Err(error)
                    if error.code() == neoengramd::CentralErrorCode::GatewayIdentityConflict =>
                {
                    let existing = self
                        .repository
                        .get_replica(&replica_id)
...
fn map_gateway_activation_error(error: GatewayReplicaActivationClientError) -> Error {
    match error {
        ...
        ) => map_central_error(neoengramd::CentralError::new(
            neoengramd::CentralErrorCode::ConcurrentUpdate,
...
fn replica_view(record: &GatewayReplicaRecord) -> GatewayReplicaView {
    GatewayReplicaView {
        ...
        supported_protocol_versions: record
            .supported_protocol_versions
            .iter()
...
fn gateway_pool_not_found() -> Error {
    map_central_error(neoengramd::CentralError::new(
        neoengramd::CentralErrorCode::GatewayPoolNotFound,
...
fn gateway_replica_not_found() -> Error {
    map_central_error(neoengramd::CentralError::new(
        neoengramd::CentralErrorCode::GatewayReplicaNotFound,
...
mod tests {
    ...
    use neoengram_protocol::{
        Ed25519PublicKeySpki, Ed25519Signature, PrincipalKind, PROTOCOL_VERSION_V1,
    };
    use neoengramd::{InMemoryClock, InMemoryGatewayRegistry};
    ...
    fn create_replica_request() -> CreateGatewayReplicaRequest {
        CreateGatewayReplicaRequest {
            ...
            supported_protocol_versions: vec![PROTOCOL_VERSION_V1.get()],
