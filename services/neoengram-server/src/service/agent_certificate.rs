...
use neoengramd::{AgentRegistryRecord, AgentRegistryService, CentralError, CentralErrorCode};
use thiserror::Error;
...
impl AgentWorkloadCertificateService {
    pub fn new(
        ...
        WorkloadIdentityUri::agent(
            &trust_domain,
            neoengram_protocol::EdgeClusterId::new("validation-edge")
                .expect("static EdgeCluster ID is valid"),
            neoengram_protocol::AgentId::new("validation-agent").expect("static Agent ID is valid"),
        )?;
        Ok(Self {
            registry,
