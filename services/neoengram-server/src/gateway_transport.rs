...
use neoengram_protocol::{
    ...
    GatewayRouteFence, GatewayRouteLeaseGranted, GatewayRouteLeaseRequest, Generation,
    ProtocolVersion, RequestId, SequenceNumber, UnixMillis, AGENT_ROUTE_LEASE_TTL_MS,
    GATEWAY_PEER_DIRECTORY_TTL_MS, MAX_AGENT_CHANNEL_FRAME_BYTES, MAX_GATEWAY_STREAM_CHUNK_BYTES,
};
use neoengramd::{
...
impl CentralGatewayControl {
    ...
    > {
        ...
        if replica.gateway_pool_id != identity.gateway_pool_id
            ...
            || hello_message.supported_protocol_versions != replica.supported_protocol_versions
...
async fn forward_agent_frame_via_authority(
    ...
    let downstream = neoengram_protocol::AgentChannelDownstreamFrame::decode_json(
        &encoded_frame[..encoded_frame.len() - 1],
    )?;
    if !matches!(
        downstream.message,
        neoengram_protocol::AgentChannelDownstreamMessage::Assignment(_)
            | neoengram_protocol::AgentChannelDownstreamMessage::Decision(_)
    ) {
        return Err(GatewaySessionError::Protocol(
            "peer forwarding accepts only Agent Assignment or Decision frames",
        ));
    }
...
fn forwarded_command_generation(
    encoded_frame: &[u8],
) -> Option<neoengram_protocol::SessionGeneration> {
    if encoded_frame.len() < 2
        || encoded_frame.len() > MAX_AGENT_CHANNEL_FRAME_BYTES.saturating_add(1)
        || encoded_frame.last() != Some(&b'\n')
    {
...
impl CentralGatewaySession {
    ...
    async fn accept_inner(
        ...
        match frame.message {
            ...
            // Agent session opened by `AgentStreamOpen`.  Keeping the legacy wire variant lets
            // older Replicas decode the protocol, but Central must fail closed before touching
            // the independent route repository operation.
    ...
    async fn send_route_fenced_error(
        &self,
        request_id: RequestId,
        trace_id: Option<neoengram_protocol::TraceId>,
        error: CentralError,
    ) -> Result<(), GatewaySessionError> {
    ...
    async fn record_heartbeat(&self) -> Result<(), GatewaySessionError> {
        ...
        replica.resource_version = neoengram_protocol::ResourceVersion::new(
            expected
                .checked_add(1)
...
pub enum GatewaySessionError {
    ...
    #[error("Gateway protocol rejected: {0}")]
    Wire(#[from] neoengram_protocol::ProtocolError),
    #[error("Gateway registry rejected the operation: {0}")]
    Registry(#[from] neoengramd::CentralError),
}
...
mod tests {
    ...
    use neoengram_protocol::{
        ...
        GatewayReplicaHeartbeat, Generation, JobDecision, JobId, JobState, MessageId, PrincipalId,
        PrincipalKind, PrincipalRef, PublishDecision, RequestId, ResourceVersion, RouteGeneration,
        SessionGeneration, TraceId, PROTOCOL_VERSION_V1,
    };
    use neoengramd::{
    ...
    async fn forward_agent_frame_via_uses_the_authoritative_owner_route_and_endpoint() {
        ...
        let (session, mut output) = control
            ...
            .await
            .unwrap();
        let agent_id = neoengram_protocol::AgentId::new("agent-a").unwrap();
    ...
    async fn peer_directory_snapshot_binds_current_certificate_and_excludes_revoked_replica() {
        ...
        assert_eq!(
            ...
            neoengram_protocol::GATEWAY_PEER_DIRECTORY_TTL_MS
        );
    ...
    fn hello_message() -> neoengram_protocol::GatewayReplicaHello {
        neoengram_protocol::GatewayReplicaHello {
            edge_cluster_id: cluster_id(),
            software_version: "0.2.0".to_owned(),
