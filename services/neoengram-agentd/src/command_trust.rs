...
use neoengram_protocol::{
    ...
    CertificateGeneration, ControlEnvelope, ControlMessage, Ed25519PublicKeySpki, UnixMillis,
};
...
impl CentralCommandTrustBundle {
    ...
    /// Verifies a polled envelope before dispatch. Non-command messages remain unsigned.
    pub(crate) fn verify_envelope_if_command(
        &self,
        ...
        let signature = envelope.central_signature.as_ref().ok_or_else(|| {
            command_rejected("Central Assignment/Decision is missing its command signature")
        })?;
        verify_signature(
            signature,
            envelope.central_command_payload_bytes(),
            self,
            now_unix_ms,
        )
    }
    ...
    pub(crate) fn verify_downstream_if_command(
        ...
        let signature = frame.central_signature.as_ref().ok_or_else(|| {
            command_rejected(
                "Central Assignment/Decision channel frame is missing its command signature",
            )
        })?;
        verify_signature(
            signature,
...
fn verify_signature(
    ...
    payload: neoengram_protocol::ProtocolResult<Vec<u8>>,
...
mod tests {
    ...
    use neoengram_protocol::{
        ...
        IndexRevision, JobDecision, JobId, JobState, MessageId, ProtocolVersion, PublishDecision,
        SequenceNumber, SessionGeneration, WireIndexVersion,
    };
    ...
    fn decision_envelope() -> ControlEnvelope {
        ControlEnvelope {
            protocol_version: ProtocolVersion::V1,
            message_id: MessageId::new("polled-central-decision-a").unwrap(),
            session_generation: SessionGeneration::new(3),
            ...
            sent_at_unix_ms: UnixMillis::new(1_000),
            central_signature: None,
            message: ControlMessage::Decision(decision()),
            extensions: Extensions::new(),
        }
    }
    ...
    fn active_and_retiring_keys_verify_exact_command_payloads() {
        ...
        let mut envelope = decision_envelope();
        envelope.central_signature = Some(signed_payload(
        ...
        trust_bundle(&key, 1, TrustKeyState::Active)
            .verify_envelope_if_command(&envelope, UnixMillis::new(1_001))
            .unwrap();
