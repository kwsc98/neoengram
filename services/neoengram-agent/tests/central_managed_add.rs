...
use neoengram_domain::protocol::{
    new_control_envelope, AgentId, AgentMountId, ArtifactId, ArtifactPlacementId,
    AssignmentGeneration, AssignmentId, AssignmentOperation, CommitDataLayout, ControlMessage,
...
async fn agent_and_control_plane_complete_managed_add_with_replayed_boundaries() {
    ...
    let mut spec = AddJobSpec {
        job_id: neoengram_domain::protocol::JobId::new("job-e2e").unwrap(),
    ...
    assert!(central
        ...
        .unwrap()
        .iter()
        .all(neoengram_central::StagedMetadataBatch::is_complete));
...
async fn agent_failure_round_trips_over_wire_into_the_control_plane() {
    ...
    let expected = central
        ...
        .unwrap();
    let mut spec = AddJobSpec {
        job_id: neoengram_domain::protocol::JobId::new("job-failure-e2e").unwrap(),
        principal: actor.clone(),
        tenant_id: tenant_id.clone(),
        project_id,
        artifact_id,
...
impl ReportSink for CentralReportSink {
    fn send(&self, report: LocalReport) -> AgentResult<()> {
        let envelope = new_control_envelope(
            RequestId::new("message-agent-e2e").map_err(protocol_report_error)?,
            TraceId::new("trace-agent-e2e").map_err(protocol_report_error)?,
            Some(self.tenant_id.clone()),
            Some(SessionGeneration::new(1)),
            UnixMillis::new(NOW + 100_000),
            report.into_control_message(),
        );
        let encoded = serde_json::to_vec(&envelope)
            .map_err(|error| AgentError::new(AgentErrorCode::ReportFailed, error.to_string()))?;
        let decoded = neoengram_domain::protocol::decode_control_envelope(&encoded)
            .map_err(protocol_report_error)?;
        let report = match decoded.body {
...
impl CentralMetadataTransfer {
    ...
    fn finalizations(
        &self,
    ) -> AgentResult<Vec<(AssignmentKey, neoengram_domain::protocol::JobDecision)>> {
...
impl ObjectTransfer for CentralMetadataTransfer {
    ...
    fn finalize(
        ...
        assignment: &neoengram_domain::protocol::AddAssignment,
        prepared: &PreparedAdd,
        decision: &neoengram_domain::protocol::JobDecision,
    ) -> AgentResult<()> {
        self.inner.finalize(assignment, prepared, decision)
    }
}

fn transfer_error(error: neoengram_central::CentralError) -> AgentError {
    AgentError::new(AgentErrorCode::ObjectTransferFailed, error.to_string())
}

fn protocol_report_error(error: neoengram_domain::protocol::ProtocolError) -> AgentError {
...
fn metadata_for(assignment: &neoengram_domain::protocol::AddAssignment) -> TestMetadata {
    ...
    let manifest = Manifest::new(object_size, ChunkingStrategy::FastCdc, vec![chunk]).unwrap();
