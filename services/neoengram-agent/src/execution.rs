...
use neoengram_domain::protocol::{
    ...
    WorkspaceMaterializeAssignment, MAX_RECORDS_PER_PAGE,
};
...
impl WorkspaceMaterializer {
    ...
    pub fn materialize(
        ...
        match rename_no_replace(&staging, &destination) {
            Ok(()) => Ok(stats),
            Err(error) if error.code() == neoengram_runtime::engine::ErrorCode::AlreadyExists => {
...
fn object_materialization_error(
    chunk: &ChunkRef,
    error: neoengram_runtime::engine::EngineError,
) -> AgentError {
    let message = match error.code() {
        neoengram_runtime::engine::ErrorCode::ObjectMissing => format!(
            "required Volume object {} for offset {} is missing",
            chunk.object_id, chunk.offset
        ),
        neoengram_runtime::engine::ErrorCode::ObjectCorrupt => format!(
    ...
    fn scan_files(
        ...
        let next = if has_more {
            let last = items.last().ok_or_else(|| {
                EngineError::new(
                    neoengram_runtime::engine::ErrorCode::IntegrityViolation,
...
pub trait ExecutionBridge: std::fmt::Debug + Send + Sync {
    ...
    fn snapshot_delivery_snapshot(
        ...
        Err(AgentError::new(
            AgentErrorCode::InvalidState,
            "Snapshot delivery metadata bridge is not configured",
        ))
    }

    /// Loads an immutable Snapshot directly for the S3 data plane. This is intentionally
    /// separate from SnapshotDelivery so deleting a local projection cannot revoke S3 reads.
    fn snapshot_read_snapshot(
        ...
        Err(AgentError::new(
            AgentErrorCode::InvalidState,
            "S3 Snapshot metadata bridge is not configured",
        ))
    }
...
impl AddExecutor for FilesystemExecution {
    fn prepare_add(&self, assignment: &AddAssignment) -> AgentResult<PreparedAdd> {
        ...
        let request = PrepareAddRequest {
            ...
            chunking: ChunkingSelection::Explicit(assignment.data_layout.into()),
            max_whole_file_bytes: (assignment.data_layout
                == neoengram_domain::protocol::CommitDataLayout::WholeFile)
                .then_some(assignment.max_whole_file_bytes.get()),
...
impl ObjectTransfer for FilesystemExecution {
    fn transfer(
        ...
        for spec in &prepared.object_specs {
            if assignment.data_layout == neoengram_domain::protocol::CommitDataLayout::FastCdc
                && spec.size > MAX_AGENT_OBJECT_BYTES
            {
                return Err(transfer_error(format!(
                    "FastCDC object {} is {} bytes, exceeding the {} byte chunk limit",
...
fn object_receipt_id(
    ...
    let canonical = neoengram_domain::protocol::domain_separated_jcs_bytes(
        "neoengram.object-receipt-id.v1",
        &scope,
    )
    .map_err(|error| transfer_error(error.to_string()))?;
...
mod tests {
    ...
    fn duplicate_payload_is_durable_once_on_the_volume_and_stages_exact_metadata_kinds() {
        ...
        for kind in [
            neoengram_domain::protocol::MetadataBatchKind::Manifest,
            neoengram_domain::protocol::MetadataBatchKind::IndexDelta,
            neoengram_domain::protocol::MetadataBatchKind::ObjectReceipt,
        ] {
            assert_eq!(
                receipt
                    .metadata_batches
    ...
    fn receipt_identity_is_stable_and_unique_across_artifact_job_scopes() {
        ...
        other_job.job_id = neoengram_domain::protocol::JobId::new("job-b").unwrap();
