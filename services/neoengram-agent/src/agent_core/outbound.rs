...
use neoengram_domain::protocol::{jcs_blake3, JobId, MessageId, TenantId, UnixMillis};
...
CREATE TABLE IF NOT EXISTS outbound_reports (
...
CREATE INDEX IF NOT EXISTS outbound_reports_job_order ON outbound_reports (job_id, sequence);
"#;
...
fn report_job_id(report: &AgentReport) -> AgentResult<neoengram_domain::protocol::JobId> {
    match report {
        AgentReport::Accepted(value) => Ok(value.job_id.clone()),
        AgentReport::Progress(value) => Ok(value.job_id.clone()),
        AgentReport::Prepared(value) => Ok(value.job_id.clone()),
        AgentReport::Finalized(value) => Ok(value.job_id.clone()),
        AgentReport::Failed(value) => Ok(value.job_id.clone()),
        // Lifecycle assignments intentionally use the durable assignment identity as their
        // queue partition key; it is already a validated JobId-compatible resource identifier.
        AgentReport::Lifecycle(value) => {
...
mod tests {
    ...
    fn sqlite_queue_is_ordered_idempotent_and_reopenable() {
        ...
        let config = SqliteOutboundReportQueueConfig::new(
            temporary.path(),
            neoengram_domain::protocol::AgentId::new("agent-a").unwrap(),
