...
use neoengram_runtime::engine::PreparedAdd;
    ...
    if !assignment.all {
        for mutation in prepared.index_delta.iter() {
            let path = match mutation {
                neoengram_domain::core::IndexMutation::Upsert { record } => &record.path,
                neoengram_domain::core::IndexMutation::Delete { path } => path,
            };
            if !assignment
                .paths
                .iter()
