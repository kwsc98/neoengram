...
impl SingleVolumeAgentConfig {
    ...
    /// Applies all node-local identity and generation fences before a destructive lifecycle
    /// command may be claimed. The command never supplies a host path; canonical relative roots
    /// are derived by the protocol type after this complete binding succeeds.
    pub fn validate_lifecycle_assignment(
        &self,
        ...
        if assignment.agent_id != self.agent_id
            || assignment.edge_cluster_id != self.edge_cluster_id
            || assignment.assignment.tenant_id != self.tenant_id
            || assignment.resource_scope.storage_volume_id() != &self.storage_volume_id
            || assignment.agent_mount_id != self.agent_mount_id
            || assignment.volume_marker_id != self.expected_volume_marker
        {
            return Err(AgentError::new(
                AgentErrorCode::ScopeMismatch,
                "lifecycle assignment does not target this Agent's complete Tenant/Volume binding",
            ));
        }
        if assignment.session_generation != expected_session_generation
            || assignment.mount_generation != self.mount_generation
            || assignment.owner_generation != self.owner_generation
        {
            return Err(AgentError::new(
                AgentErrorCode::GenerationMismatch,
                "lifecycle assignment carries a stale session, mount, or owner generation",
            ));
        }
        assignment
            .resource_scope
            .canonical_relative_roots(&self.tenant_id)
            .map_err(AgentError::from)?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn probe(marker: &str) -> SingleVolumeMountProbe {
        SingleVolumeMountProbe {
            ...
            mount_identity_digest: AgentMountIdentityDigest::new(
                neoengram_domain::core::ContentDigest::from_bytes([0x42; 32]),
            ),
    ...
    #[test]
    fn lifecycle_assignment_requires_the_complete_live_binding() {
        let configured = config();
        let session_generation = SessionGeneration::new(9);
        let mut command = AgentResourceLifecycleAssignment {
            assignment: neoengram_domain::protocol::ResourceLifecycleAssignment {
                assignment_id: neoengram_domain::protocol::LifecycleAssignmentId::new("life-1")
                    .unwrap(),
                ...
                lifecycle_generation: neoengram_domain::protocol::LifecycleGeneration::new(2),
            ...
            resource_scope: neoengram_domain::protocol::AgentResourceLifecycleScope::Playground {
                ...
                .unwrap(),
                placement_generation: neoengram_domain::protocol::PlacementGeneration::new(2),
            },
            agent_id: configured.agent_id.clone(),
            edge_cluster_id: configured.edge_cluster_id.clone(),
            agent_mount_id: configured.agent_mount_id.clone(),
            volume_marker_id: configured.expected_volume_marker.clone(),
            session_generation,
            mount_generation: configured.mount_generation,
            owner_generation: configured.owner_generation,
            extensions: Extensions::new(),
        };

        configured
            .validate_lifecycle_assignment(&command, session_generation, UnixMillis::new(1_000))
            .unwrap();
        command.owner_generation = OwnerGeneration::new(99);
        assert_eq!(
            configured
