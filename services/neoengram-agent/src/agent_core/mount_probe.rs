...
/// Rechecks the managed Volume marker immediately before a destructive operation.
///
/// The Unix implementation opens both the root and marker with `O_NOFOLLOW`, so replacing either
/// path with a symlink cannot redirect lifecycle cleanup outside the approved Volume.
pub fn verify_volume_marker(root: &Path, expected: &VolumeMarkerId) -> AgentResult<()> {
    #[cfg(unix)]
    {
        let observed = read_marker(root).map_err(|()| {
            AgentError::new(
                AgentErrorCode::MountUnavailable,
                "managed Volume marker is unavailable or invalid",
            )
        })?;
        if &observed != expected {
            return Err(AgentError::new(
                AgentErrorCode::ScopeMismatch,
                "managed Volume marker does not match the lifecycle assignment",
            ));
        }
        Ok(())
    }
    #[cfg(not(unix))]
    {
        ...
        Err(AgentError::new(
            AgentErrorCode::MountUnavailable,
            "secure Volume marker verification is unavailable on this platform",
        ))
    }
}
...
mod tests {
    ...
    fn marker_must_be_regular_and_exact() {
        ...
        verify_volume_marker(root.path(), &VolumeMarkerId::new("marker-a").unwrap()).unwrap();
        assert_eq!(
            verify_volume_marker(root.path(), &VolumeMarkerId::new("marker-b").unwrap())
                .unwrap_err()
                .code(),
            AgentErrorCode::ScopeMismatch
        );
