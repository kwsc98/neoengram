    ...
    async fn post<T: Serialize>(
        ...
        let response = client
            ...
            .map_err(|error| {
                EnrollmentClientError::new(
                    true,
                    format!("Gateway enrollment endpoint is unavailable: {error}"),
                )
            })?;
            ...
            .and_then(|value| value.parse::<usize>().ok())
        {
            if length > MAX_AGENT_BODY_BYTES {
                return Err(EnrollmentClientError::new(
                    false,
                    "Gateway enrollment response exceeds the transport limit",
                ));
            }
        }
...
async fn read_bounded(mut response: reqwest::Response) -> Result<Vec<u8>, EnrollmentClientError> {
    ...
    while let Some(chunk) = response.chunk().await.map_err(|error| {
        EnrollmentClientError::new(true, format!("Gateway enrollment response failed: {error}"))
    })? {
...
fn validate_response_request_id(
    ...
    let observed = response
        ...
        .and_then(|value| value.to_str().ok())
        .ok_or_else(|| {
            EnrollmentClientError::new(false, "Gateway enrollment response omitted X-Request-ID")
        })?;
    let observed = RequestId::new(observed).map_err(|_| {
        EnrollmentClientError::new(
            false,
            "Gateway enrollment response returned invalid X-Request-ID",
        )
    })?;
    if &observed != expected {
        return Err(EnrollmentClientError::new(
            false,
            "Gateway enrollment response returned a different X-Request-ID",
        ));
    }
    Ok(())
}
...
fn require_json_content_type(content_type: &str) -> Result<(), EnrollmentClientError> {
    ...
    if media_type == JSON_CONTENT_TYPE {
        ...
        Err(EnrollmentClientError::new(
            false,
            "Gateway enrollment success response is not application/json",
        ))
    }
}
...
fn remote_error(response: HttpResponse) -> EnrollmentClientError {
    ...
    EnrollmentClientError::new(
        retryable,
        format!(
            "Gateway enrollment returned HTTP {} ({code})",
