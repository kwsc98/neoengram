...
pub(crate) fn spawn_work_dispatcher(
    ...
    let task = tokio::spawn(async move {
        ...
        let mut workers = tokio::task::JoinSet::<(String, AgentDaemonResult<()>)>::new();
        loop {
            ...
            tokio::select! {
                item = receiver.recv() => {
                    ...
                    queued.entry(item.work.work_key()).or_default().push_back(item);
                }
...
mod tests {
    use neoengram_domain::protocol::{
        ControlError, ErrorCode, Extensions, MessageId, SequenceNumber, SessionGeneration,
        UnixMillis, CURRENT_WIRE_VERSION, MAX_AGENT_CHANNEL_FRAME_BYTES,
    };
