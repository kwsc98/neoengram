...
impl AgentConfig {
    ...
    pub fn validate(&self) -> AgentDaemonResult<()> {
        ...
        if self.wire_version != CURRENT_WIRE_VERSION.get() {
            return Err(configuration(format!(
                "wire_version must be {}",
                CURRENT_WIRE_VERSION.get()
            )));
        }
...
impl StorageConfig {
    fn validate(&self, storage_volume_id: &StorageVolumeId) -> AgentDaemonResult<()> {
        ...
        let expected_marker_file = self.mount_path.join(crate::VOLUME_MARKER_FILE_NAME);
...
impl RegistrationConfig {
    fn validate(&self) -> AgentDaemonResult<()> {
        if !self.approval_required {
            return Err(configuration(
                "registration.approval_required must be true in enrollment current wire protocol",
            ));
        }
