...
| R6 | 后续 | 客户端 push、fetch、clone 与授权训练读取 | P2 / P3 |
| R7 | 后续 | Playground mutation、Volume Owner、lease 与强 fencing | A3 |
| R8 | 后续 | 生命周期、GC、HA、灾备、配额和规模优化 | P4 / P5 / A5 |
| R9 | 研究 | mode、symlink、ACL/xattr、sparse 等文件语义 | P6 |
...
当前格式只接受 `application_id = 0x4e454f41`、`user_version = 1`；不探测、导入或迁移旧格式。
...
| R3 | H2 JSON sequence 经目标 Ingress 的双向流、背压和重连原型 |
| R4 | 目标 S3 的 checksum、multipart、Signed URL、KMS 和失败清理原型 |
| R7 | 首批 NFS 产品矩阵、强 fencing 方案及 RW Playground Pod 策略 |
| R8 | 千万路径 Delta、上亿对象 catalog 和恢复时间的可重复基准 |
