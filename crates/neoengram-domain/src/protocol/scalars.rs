...
/// The numeric revision carried by the current wire protocol.
///
/// Serde deliberately accepts every `u16` so runtime validation can map a non-current value to the
/// stable `PROTOCOL_UNSUPPORTED` error. Published current JSON Schemas expose only version `1`.
...
impl JsonSchema for ProtocolVersion {
    ...
    fn schema_id() -> Cow<'static, str> {
        concat!(module_path!(), "::ProtocolVersion::current").into()
    }

    fn json_schema(_generator: &mut SchemaGenerator) -> Schema {
        schemars::json_schema!({
            "description": "The only wire version accepted by the current NeoEngram protocol.",
            "type": "integer",
            "const": 1
        })
    }
