...
impl AgentEnrollmentEnvelope {
    ...
    pub fn validate(&self) -> ProtocolResult<()> {
        ...
        validate_extension_keys(&self.extensions, &["wire_version", "type", "payload"])?;
...
mod tests {
    ...
    #[test]
    fn bootstrap_rejects_non_current_wire_version() {
        let key_pair = test_key_pair();
        let mut request = bootstrap_request(&key_pair);
        request.wire_version = ProtocolVersion::new(2);
        assert!(matches!(
            request.validate(),
            Err(ProtocolError::InvalidField {
                field: "wire_version",
    ...
    fn envelope_rejects_duplicate_members() {
        let bytes = br#"{"wire_version":1,"wire_version":1,"type":"agent.enrollment.token.create","payload":{}}"#;
    ...
    fn bootstrap_proof_detects_tampering() {
        ...
        assert_eq!(
            ...
            "412252b722e90a7f0030cc7977951d92962a3ddf8d10244a504a18ae93e4afa9"
        );
    ...
    #[test]
    fn bootstrap_rejects_unknown_extension_members() {
        let key_pair = test_key_pair();
        let mut wire = serde_json::to_value(bootstrap_request(&key_pair)).unwrap();
        wire.as_object_mut()
            .unwrap()
            .insert("x_policy_hint".to_owned(), serde_json::json!(true));
        assert!(AgentBootstrapRequest::decode_json(&serde_json::to_vec(&wire).unwrap()).is_err());
    }
    ...
    #[test]
    fn status_rejects_unknown_extension_members() {
        let key_pair = test_key_pair();
        let mut wire = serde_json::to_value(signed_status_request(&key_pair)).unwrap();
        wire.as_object_mut()
            .unwrap()
            .insert("x_status_hint".to_owned(), serde_json::json!(true));
        assert!(
            AgentBootstrapStatusRequest::decode_json(&serde_json::to_vec(&wire).unwrap()).is_err()
        );
    }
    ...
    fn direct_decoders_reject_nested_duplicates_and_oversized_input() {
        ...
        let nested_duplicate =
            br#"{"wire_version":1,"proof":{"algorithm":"ed25519","algorithm":"ed25519"}}"#;
