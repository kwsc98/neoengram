...
use super::validation::{parse_unique_json, CONTENT_DIGEST_PATTERN};
use crate::{
    ...
    GatewayReplicaId, Generation, LifecycleGeneration, ProtocolError, ProtocolResult,
    ProtocolVersion, RequestId, ResourceVersion, RouteGeneration, SequenceNumber,
    SessionGeneration, SnapshotId, TenantId, TraceId, UnixMillis, CURRENT_WIRE_VERSION,
};
...
impl GatewayAgentAction {
    const fn registry_kind(self) -> crate::AgentActionKind {
        match self {
            Self::EnrollmentBootstrap => crate::AgentActionKind::EnrollmentBootstrap,
            Self::EnrollmentStatusQuery => crate::AgentActionKind::EnrollmentStatusQuery,
            Self::SessionOpen => crate::AgentActionKind::SessionOpen,
            Self::SessionChannelOpen => crate::AgentActionKind::SessionChannelOpen,
            Self::SessionHeartbeatReport => crate::AgentActionKind::SessionHeartbeatReport,
            Self::JobReportCreate => crate::AgentActionKind::JobReportCreate,
            Self::JobMetadataBatchStage => crate::AgentActionKind::JobMetadataBatchStage,
            Self::JobMetadataPageStage => crate::AgentActionKind::JobMetadataPageStage,
            Self::JobIndexPageQuery => crate::AgentActionKind::JobIndexPageQuery,
            Self::JobManifestPageQuery => crate::AgentActionKind::JobManifestPageQuery,
            Self::SessionClose => crate::AgentActionKind::SessionClose,
        }
    }
    ...
    pub fn from_path(path: &str) -> Option<Self> {
        match crate::agent_action_from_path(path)? {
            crate::AgentActionKind::EnrollmentBootstrap => Some(Self::EnrollmentBootstrap),
            crate::AgentActionKind::EnrollmentStatusQuery => Some(Self::EnrollmentStatusQuery),
            crate::AgentActionKind::SessionOpen => Some(Self::SessionOpen),
            crate::AgentActionKind::SessionChannelOpen => Some(Self::SessionChannelOpen),
            crate::AgentActionKind::SessionHeartbeatReport => Some(Self::SessionHeartbeatReport),
            crate::AgentActionKind::JobReportCreate => Some(Self::JobReportCreate),
            crate::AgentActionKind::JobMetadataBatchStage => Some(Self::JobMetadataBatchStage),
            crate::AgentActionKind::JobMetadataPageStage => Some(Self::JobMetadataPageStage),
            crate::AgentActionKind::JobIndexPageQuery => Some(Self::JobIndexPageQuery),
            crate::AgentActionKind::JobManifestPageQuery => Some(Self::JobManifestPageQuery),
            crate::AgentActionKind::SessionClose => Some(Self::SessionClose),
        }
    }
}
...
/// Central's monotonic fence for already-issued S3 read tickets.
///
/// Both generations are included because Snapshot lifecycle changes and Access Point policy
/// changes are independent authorization boundaries. A Gateway rejects a ticket when either of
/// its generations is older than the latest fence it has observed for the same resource.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct GatewayS3ReadRevocation {
    pub tenant_id: TenantId,
    pub snapshot_id: SnapshotId,
    pub minimum_snapshot_lifecycle_generation: LifecycleGeneration,
    #[schemars(length(min = 1, max = 63))]
    pub bucket: String,
    pub minimum_access_point_policy_generation: ResourceVersion,
    #[schemars(length(min = 1, max = 1024))]
    pub reason: String,
}
...
pub enum GatewayControlMessage {
    ...
    #[serde(rename = "s3_read_revocation")]
    S3ReadRevocation(GatewayS3ReadRevocation),
...
fn validate_message(message: &GatewayControlMessage) -> ProtocolResult<()> {
    match message {
        GatewayControlMessage::ReplicaHello(hello) => {
            ...
            if hello.wire_version != CURRENT_WIRE_VERSION {
                return invalid(
                    "wire_version",
                    "Gateway Replica must use the current wire version",
                );
            }
        ...
        GatewayControlMessage::S3ReadRevocation(revocation) => {
            if revocation.minimum_snapshot_lifecycle_generation.get() == 0 {
                return invalid(
                    "minimum_snapshot_lifecycle_generation",
                    "Snapshot lifecycle revocation generation must be positive",
                );
            }
            if revocation.minimum_access_point_policy_generation.get() == 0 {
                return invalid(
                    "minimum_access_point_policy_generation",
                    "Access Point policy revocation generation must be positive",
                );
            }
            validate_text_max("bucket", &revocation.bucket, 63)?;
            validate_text("reason", &revocation.reason)
        }
...
mod tests {
    ...
    #[test]
    fn decode_rejects_unknown_envelope_extensions() {
    ...
    #[test]
    fn hello_requires_exactly_the_current_wire_version() {
    ...
    #[test]
    fn hello_rejects_non_current_wire_version() {
        let hello = GatewayReplicaHello {
            edge_cluster_id: crate::EdgeClusterId::new("cluster-a").unwrap(),
            software_version: "0.2.0".to_owned(),
        ...
        assert!(matches!(
            frame(GatewayControlMessage::ReplicaHello(hello)).validate(),
            Err(ProtocolError::InvalidField {
                field: "wire_version",
                ..
            })
