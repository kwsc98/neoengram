...
//! canonical format used by the current repository format.
...
impl IndexSnapshotHasher {
    /// Starts an empty WorkspaceIndex snapshot digest for the current repository format.
    pub fn new() -> Self {
        let mut hasher = canonical_hasher(INDEX_SNAPSHOT_HASH_DOMAIN);
        hasher.update(&WORKSPACE_INDEX_FORMAT_VERSION.to_le_bytes());
        Self {
            hasher,
            previous_path: None,
