...
use synapse::{Commit, FileNode, Index, ObjectStore, Tree, INDEX_FORMAT_VERSION};
...
pub(crate) const SYNAPSE_DIR_NAME: &str = ".synapse";
...
const COMMIT_HASH_DOMAIN: &[u8] = b"synapse-commit-v2";
const LOCK_FORMAT_VERSION: u32 = 1;
const STORAGE_SCAN_PAGE_SIZE: u32 = 1_024;

/// 保存在 `.synapse/metadata/repository.json` 中的后端选择与仓库格式配置。
...
impl Repository {
    ...
    pub(crate) fn open_or_default(root: PathBuf) -> Result<Self> {
        let synapse_dir = root.join(SYNAPSE_DIR_NAME);
        match fs::symlink_metadata(&synapse_dir) {
            Ok(metadata) => {
                ensure!(
                ...
                Self::open(root).with_context(|| {
                    format!(
                        "已有目录不是受支持的 Synapse 仓库: {}",
                        synapse_dir.display()
                    )
                })
            }
            Err(error) if error.kind() == io::ErrorKind::NotFound => Self::at(root),
            Err(error) => {
                Err(error).with_context(|| format!("无法检查仓库目录: {}", synapse_dir.display()))
            }
        }
    }

    fn open(root: PathBuf) -> Result<Self> {
        let metadata_dir = root.join(SYNAPSE_DIR_NAME).join(METADATA_DIR_NAME);
    ...
    fn with_store_kinds(
        ...
        let metadata_dir = root.join(SYNAPSE_DIR_NAME).join(METADATA_DIR_NAME);
        let objects_dir = root.join(SYNAPSE_DIR_NAME).join(OBJECTS_DIR_NAME);
        Ok(Self {
            root,
    ...
    pub(crate) fn discover(start: &Path) -> Result<Self> {
        ...
        for root in canonical_start.ancestors() {
            ...
            match fs::symlink_metadata(&synapse_dir) {
                ...
                Ok(_) => bail!("Synapse 内部路径不是目录: {}", synapse_dir.display()),
                Err(error) if error.kind() == io::ErrorKind::NotFound => {}
                Err(error) => {
                    return Err(error).with_context(|| {
                        format!("无法检查 Synapse 仓库目录: {}", synapse_dir.display())
                    });
                }
            }
        }

        bail!("当前目录不在 Synapse 仓库中，请先运行 `synapse init`")
    }
    ...
    fn synapse_dir(&self) -> PathBuf {
        self.root.join(SYNAPSE_DIR_NAME)
    }
    ...
    pub(crate) fn staging_dir(&self) -> PathBuf {
        self.synapse_dir().join(STAGING_DIR_NAME)
    }

    pub(crate) fn files_dir(&self) -> PathBuf {
        self.synapse_dir().join(FILES_DIR_NAME)
    }

    fn metadata_dir(&self) -> PathBuf {
        self.synapse_dir().join(METADATA_DIR_NAME)
    }

    pub(crate) fn transactions_dir(&self) -> PathBuf {
        self.synapse_dir().join(TRANSACTIONS_DIR_NAME)
    }
    ...
    pub(crate) fn ensure_no_unfinished_transactions(&self) -> Result<()> {
        let transactions = self.unfinished_transactions()?;
            ...
            "检测到未完成的工作区事务，请先运行 `synapse recover` 或 `synapse recover --abort`: {}",
            transactions
                .first()
...
fn validate_repository_path(path: &str) -> Result<()> {
    ...
    for component in path.split('/') {
            ...
            !component.eq_ignore_ascii_case(SYNAPSE_DIR_NAME),
            "不能记录 Synapse 内部路径: {path}"
        );
...
mod tests {
    use anyhow::Result;
    use synapse::{Commit, FileNode, Tree};
    ...
    fn current_tree_and_commit_ids_are_stable() -> Result<()> {
        ...
        assert_eq!(
            tree_id,
            "20a876281d5d4df054063618282f3ea2c4cf8424587f23f7f9fdf87ead067ea8"
        );
        ...
        assert_eq!(
            ...
            "3ea6c409cd9f3dce3475ebae0471d56fdef0ced57bd36eedbcc65e6ca49955db"
        );
        Ok(())
    }
