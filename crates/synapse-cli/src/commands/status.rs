...
use synapse::FileNode;
use walkdir::WalkDir;

use crate::repository::{Repository, SYNAPSE_DIR_NAME};
...
fn is_synapse_name(name: &OsStr) -> bool {
    name.to_str()
        .is_some_and(|name| name.eq_ignore_ascii_case(SYNAPSE_DIR_NAME))
}
