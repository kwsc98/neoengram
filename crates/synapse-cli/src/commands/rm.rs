...
use synapse::{FileNode, Index};
use tempfile::{Builder, NamedTempFile};

use crate::{
    repository::{Repository, SYNAPSE_DIR_NAME},
...
pub(crate) async fn execute(path: PathBuf, cached: bool, force: bool) -> Result<()> {
    ...
    let removed = tokio::task::spawn_blocking(move || {
        remove_tracked_path(&task_repository, &current_dir, &path, cached, force)
    })
    .await
    .context("rm 任务异常终止")??;
...
fn repository_scope(path: &Path, current_dir: &Path, repository_root: &Path) -> Result<String> {
    ...
    for (index, component) in components.iter().enumerate() {
        ...
        let name = name
            ...
            !name.eq_ignore_ascii_case(SYNAPSE_DIR_NAME),
            "不能移除 Synapse 内部路径: {}",
            path.display()
        );
        names.push(name);
