...
use synapse::{Chunk, Commit, FileNode, Index, INDEX_FORMAT_VERSION};
...
fn manifest_ids_are_stable() -> Result<()> {
    ...
    assert_eq!(
        ...
        (
            "813f5972f8652c27637a5c7b813d596882341049370349f08254b515a930a118".to_owned(),
            "7f564243dbdcf30631a2f9576f07dbb865d507dff99a6a5d7c99c56f253f2e8a".to_owned()
        )
    );
    Ok(())
}
...
fn json_backend_ignores_unpublished_atomic_temp_files() -> Result<()> {
    ...
    for path in [
        root.join("trees/.synapse-tmp-orphan"),
        root.join("commits/.synapse-tmp-orphan"),
        root.join("refs/heads/.synapse-tmp-orphan"),
    ] {
    ...
    assert!(store
        .compare_exchange_reference(
            "refs/heads/.synapse-tmp-reserved",
            None,
            Some(&"a".repeat(64)),
        )
        .is_err());
...
fn json_backend_does_not_hide_non_file_temp_entries() -> Result<()> {
    ...
    let store = JsonMetadataStore::new(root.clone());
    store.initialize("refs/heads/main")?;
    fs::create_dir(root.join("trees/.synapse-tmp-not-a-file"))?;
