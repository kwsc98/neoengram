...
use synapse::{Chunk, FileNode, Index, INDEX_FORMAT_VERSION};
...
fn an_unlocked_stale_lock_file_does_not_block_writers() -> TestResult {
    let repository = tempfile::tempdir()?;
    initialize(repository.path())?;
    let lock = repository.path().join(".synapse/metadata/write.lock");
...
fn a_symlinked_lock_file_is_rejected_without_touching_its_target() -> TestResult {
    ...
    symlink(
        outside.path(),
        repository.path().join(".synapse/metadata/write.lock"),
    )?;
...
fn read_index(repository: &Path) -> Result<Index, Box<dyn Error>> {
    let metadata_dir = repository.join(".synapse/metadata");
...
fn object_path(repository: &Path, hash: &str) -> PathBuf {
    repository.join(".synapse/objects").join(hash)
}

fn current_ref(repository: &Path) -> PathBuf {
    repository.join(".synapse/metadata/refs/heads/main")
}
