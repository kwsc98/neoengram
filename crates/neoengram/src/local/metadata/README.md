...
本目录是 `neoengram::local::metadata`，定义本地仓库的元数据持久化契约，并提供 JSON 与
SQLite 两个后端。它不承担远端元数据配置中心或服务端数据库适配；上层不暴露文件路径、
SQL 连接或物理表结构，所有本地后端共享同一套可观察契约。
...
- Chunk payload 的保存、校验和 durability barrier，这些属于 `local::objects` 中的
  `ObjectStore`；
- `repository.json` 中的仓库格式和后端选择；
- 仓库级命令写锁、工作区文件、完整文件缓存、输入 staging；
- checkout/rm 恢复 journal（属于 `local::worktree`）和命令编排（属于 `app`）；
- 路径的跨平台规范化、文件/目录祖先冲突等 `local::repository` 领域校验；
...
跨模块发布顺序和规模约束见
[`docs/storage-architecture.md`](../../../../../docs/storage-architecture.md)，完整源码依赖边界与
未来远端模块落点见 [`docs/code-architecture.md`](../../../../../docs/code-architecture.md)。
