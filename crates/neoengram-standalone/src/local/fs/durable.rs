...
pub(crate) const RESTORE_TRANSACTION_DRAFT_PREFIX: &str = ".neoengram-tmp-restore-";
...
fn directory_cleanup_prefix(path: &Path) -> &'static str {
    ...
    } else if name.starts_with("restore-") || name.starts_with(RESTORE_TRANSACTION_DRAFT_PREFIX) {
        RESTORE_TRANSACTION_DRAFT_PREFIX
    } else {
...
pub(crate) fn is_transaction_draft_name(name: &OsStr) -> Result<bool> {
    ...
    let suffix = name
        ...
        .or_else(|| name.strip_prefix(RESTORE_TRANSACTION_DRAFT_PREFIX));
    ensure!(
    ...
    F: FnOnce(&Path),
{
    let expected_prefix = match operation {
        ...
        "restore" => RESTORE_TRANSACTION_DRAFT_PREFIX,
    ...
    S: FnMut(&Path) -> Result<()>,
{
    ...
    on_renamed();
    if cfg!(debug_assertions) && crate::failure_injected("rename-after-publish-before-sync") {
