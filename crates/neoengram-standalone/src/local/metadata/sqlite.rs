...
use super::{
    ...
    ReferenceCas, RepositoryMetadataStore, StoredReference, WorkspaceId, WorkspaceIndexStore,
    WorkspaceRecord, WorkspaceRegistry, DEFAULT_WORKSPACE_ID,
};
...
fn read_index_state(
    ...
    // `index_accumulator` is the format-v8 physical column name retained for compatibility. Its
    // value is the canonical core snapshot digest, not the pre-v8 commutative accumulator.
    let digest = ContentDigest::from_bytes(decode_digest(accumulator, "Index digest")?);
    Ok(StoredIndexState {
        format,
        revision,
        count,
