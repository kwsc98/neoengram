...
NeoEngram format v8 只使用 SQLite 元数据后端。旧 JSON 后端、后端选择参数和旧仓库读取均已
删除；v7 或其他版本不匹配的仓库会被明确拒绝，当前不提供迁移工具。
...
- `WorkspaceIndexStore`：按 `workspace_id` 隔离的分页 Index、IndexVersion 和 expected-version transaction；
...
`MetadataStore` 只是迁移期把这些窄接口组合到同一个 SQLite trait object 的内部 composition surface，
不是允许上层或 Managed 模式依赖的单体存储契约。规范 ID 计算属于 `neoengram-core`；SQLite writer
只负责 staging、重复发布校验和事务持久化。

Chunk payload 由 `ObjectStore` 管理。仓库锁、工作区、mutation journal、完整文件缓存和
FUSE inode/cache 均不属于本模块。

SQLite store 可跨线程共享；Reader、Snapshot、Index transaction 和 Directory writer 只在创建
