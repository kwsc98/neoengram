...
use crate::local::{
    ...
    model::{Commit, DirectoryEntry, DirectoryEntryKind, FileNode, Index, Tree},
};
...
use neoengram_fs::MemoryIndexSnapshot;
...
impl Repository {
    ...
    /// Builds the current Index into a canonical Engine graph without publishing HEAD/ref state.
    pub(crate) fn build_commit_graph(
        ...
        let index = self.metadata_store.read_index()?;
            ...
            index.format_version() == crate::local::model::INDEX_FORMAT_VERSION,
            "不支持的 index 格式版本 {}",
            index.format_version()
        );
        ensure!(
            index.version() == &request.expected_index_version,
            "Index 在 Commit 图构造前发生变化"
        );
...
mod tests {
    ...
    use crate::local::model::{ChunkingStrategy, FileNode, Index};
    use anyhow::Result;
    ...
    fn identical_subdirectories_are_deduplicated() -> Result<()> {
        ...
        let repository = Repository::open_or_create(root)?;
        repository.initialize_layout()?;
        let tree = crate::local::model::Tree {
            files: vec![
                FileNode {
