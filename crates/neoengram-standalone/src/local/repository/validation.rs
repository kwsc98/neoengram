...
#[cfg(test)]
mod tests {
    use crate::local::model::{ChunkingStrategy, Commit, FileNode, Tree};
    use anyhow::Result;
    ...
    fn current_tree_and_commit_ids_are_stable() -> Result<()> {
        ...
        let commit = Commit {
            root_directory_id: tree_id.clone(),
            parent: None,
            message: "snapshot".to_owned(),
            created_at_unix_ms: 1,
        };
        let commit_id = repository.commit_id(&commit)?;
        let core_commit = neoengram_core::Commit::new(tree_id.parse()?, None, "snapshot", 1)?;
