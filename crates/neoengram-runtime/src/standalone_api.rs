...
///
/// Commands use private workspace views internally while composing these paged ports; callers
/// must not treat those views as cross-crate contracts.
...
static FAILURE_INJECTOR: std::sync::OnceLock<
    std::sync::Arc<dyn crate::engine::FailureInjector>,
> = std::sync::OnceLock::new();
...
pub fn install_failure_injector(
    injector: std::sync::Arc<dyn crate::engine::FailureInjector>,
) -> bool {
...
pub(crate) fn failure_injected(point: &'static str) -> bool {
    FAILURE_INJECTOR.get().is_some_and(|injector| {
        injector
            .check(crate::engine::FailurePoint::Named(point))
            .is_err()
    })
}
...
///
/// Execution progress uses [`crate::engine::ProgressSink`]. Keeping diagnostics separate
...
pub enum HeadPosition {
    Branch(String),
    Detached(neoengram_domain::core::CommitId),
}

/// Structured status observation over HEAD, WorkspaceIndex, and the worktree.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub struct CommitRecord {
    pub id: neoengram_domain::core::CommitId,
    pub commit: neoengram_domain::core::Commit,
}
...
/// File metadata exposed by show without leaking the local directory representation.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ShowFileRecord {
    pub path: String,
    pub total_size: u64,
    pub chunk_count: u64,
    pub chunking: neoengram_domain::core::ChunkingStrategy,
}
...
pub struct AddedFile {
    ...
    pub path: neoengram_domain::core::LogicalPath,
...
/// Result returned after a Standalone Add has either published its WorkspaceIndex CAS or made no changes.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct AddResult {
    ...
    /// Current WorkspaceIndex version observed after the operation.
    pub index_version: neoengram_domain::core::IndexVersion,
    /// Whether this invocation published a new WorkspaceIndex version.
...
pub struct CommitResult {
    ...
    pub commit_id: neoengram_domain::core::CommitId,
    /// Canonical identifier of the Commit's root Directory.
    pub root_directory_id: neoengram_domain::core::DirectoryId,
...
/// Result of atomically replacing the workspace WorkspaceIndex, worktree, and HEAD position.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Result of removing tracked paths from the standalone WorkspaceIndex.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RemoveResult {
    /// Canonical paths removed from the WorkspaceIndex, in stable operation order.
    pub removed_paths: Vec<neoengram_domain::core::LogicalPath>,
...
pub enum RestoreTarget {
    /// Restore the selected WorkspaceIndex entries from HEAD.
    WorkspaceIndex,
    /// Restore selected worktree files from the WorkspaceIndex.
    Worktree,
}
...
pub struct RestoreResult {
    ...
    pub paths: Vec<neoengram_domain::core::LogicalPath>,
...
pub enum RecoveryOutcome {
    ...
    Checkout {
        ...
        target: neoengram_domain::core::CommitId,
...
pub struct InitResult {
    ...
    /// Whether a current metadata format repository already existed at the requested location.
...
pub struct WorkspaceSummary {
    ...
    pub head: HeadPosition,
    /// Commit on which the workspace WorkspaceIndex is based, when one exists.
    pub base_commit_id: Option<neoengram_domain::core::CommitId>,
...
pub mod commands {
    ...
    use anyhow::Result;
    use neoengram_domain::core::ChunkingStrategy;
    ...
    pub use crate::engine::{NoopProgressSink, ProgressEvent, ProgressSink};
