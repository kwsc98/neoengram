...
pub(crate) fn restore_worktree_paths(
    ...
    let files: Vec<WorkspaceFileRecord> = index
        .files
        .into_iter()
        .filter(|file| {
            scopes
    ...
    for file in &files {
        ...
        if let Some(metadata) = metadata {
            ...
            if file_matches_record(&destination, file)? {
                continue;
            }
            ensure!(
                force,
...
impl RestoreTransaction {
    fn create(repository: &Repository, files: &[WorkspaceFileRecord]) -> Result<Self> {
    ...
    if !abort && transaction.journal.phase == RestorePhase::Applied {
        for entry in &transaction.journal.entries {
            let destination = workspace_path(repository.root(), &entry.file.path);
                ...
                file_matches_record(&destination, &entry.file)?,
