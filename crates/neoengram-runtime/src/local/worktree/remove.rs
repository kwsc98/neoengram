...
use crate::local::{
    ...
    model::{WorkspaceFileRecord, WorkspaceIndex},
...
fn remove_workspace_and_update_index(
    repository: &Repository,
    removed_nodes: &[WorkspaceFileRecord],
    previous_index: &WorkspaceIndex,
    next_index: &WorkspaceIndex,
    ...
    let actions = removed_nodes
        ...
        .collect::<neoengram_domain::core::ValidationResult<Vec<_>>>()?;
    ...
    executed.publish_and_finalize(|transaction| {
        ...
        // WorkspaceIndex CAS 已生效后绝不能回滚工作区；后续失败由 recover 完成收尾。
...
fn ensure_workspace_is_clean(repository_root: &Path, file: &WorkspaceFileRecord) -> Result<()> {
        ...
        if !force {
            // 在真正 rename 前再次核对内容，缩小预检与修改工作区之间的竞态窗口。
                ...
                metadata.is_file()
                    ...
                    && file_matches_record(&original, file)?,
                "工作区文件在 rm 执行期间发生变化: {}",
                file.path
            );
        }
...
pub(super) fn recover_transaction(
    ...
    let before: WorkspaceIndex = read_json(&transaction_root.join("index.before.json"))?;
    let after: WorkspaceIndex = read_json(&transaction_root.join("index.after.json"))?;
...
fn validate_manifest(
    ...
    let before_files: std::collections::BTreeMap<&str, &WorkspaceFileRecord> = before
        .files
        .iter()
        .map(|file| (file.path.as_str(), file))
        .collect();
    let after_files: std::collections::BTreeMap<&str, &WorkspaceFileRecord> = after
        .files
        .iter()
        .map(|file| (file.path.as_str(), file))
        .collect();
