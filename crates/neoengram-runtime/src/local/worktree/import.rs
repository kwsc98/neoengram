...
use crate::local::{
    model::{Chunk, ChunkingStrategy, WorkspaceFileRecord},
    objects::{ObjectSpec, ObjectStore},
};
...
#[cfg(test)]
fn chunk_file_blocking(path: &Path, objects_dir: &Path) -> Result<WorkspaceFileRecord> {
...
mod tests {
    ...
    use crate::local::model::ChunkingStrategy;
    ...
    async fn empty_file_has_no_payload_chunks() -> Result<()> {
        ...
        let file_record = chunk_file(source, store, staging_dir, ChunkingStrategy::FastCdc).await?;

        assert_eq!(file_record.total_size, 0);
        assert!(file_record.chunks.is_empty());
