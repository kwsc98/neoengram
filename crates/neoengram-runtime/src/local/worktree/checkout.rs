...
use crate::engine::{MutationAction, ProgressSink};
use anyhow::{bail, ensure, Context, Result};
use neoengram_domain::core::{LogicalPath, ManifestId};
...
use crate::local::{
    ...
    model::{WorkspaceFileRecord, WorkspaceIndex, WORKSPACE_INDEX_FORMAT_VERSION},
    objects::ObjectSpec,
    repository::{Repository, RepositoryWriteLock},
};

use super::workspace::{file_matches_record, logical_join, safe_leaf_metadata, workspace_path};
...
pub(crate) fn checkout_snapshot(
    ...
    checkout_snapshot_with_progress(repository, target, force, &crate::engine::NoopProgressSink)
}
...
fn checkout_snapshot_locked(
    ...
    if !force {
        let head_commit = repository.read_commit(head_id)?;
            ...
            repository.directory_id(&current_index.files)? == head_commit.root_directory_id,
    ...
    let target_tree = repository.read_directory_files(&target_commit.root_directory_id)?;
...
fn apply_checkout_transaction_with_engine(
    repository: &Repository,
    current_index: &WorkspaceIndex,
    target_tree: &[WorkspaceFileRecord],
    expected_index_version: neoengram_domain::core::IndexVersion,
    mutation_kind: &str,
    force: bool,
...
fn publish_checkout_authority(
    ...
    let target_index = WorkspaceIndex {
        format_version: WORKSPACE_INDEX_FORMAT_VERSION,
        files: target_tree.to_vec(),
    };
        ...
        match repository.read_index() {
            ...
            Ok(_) => {
                return Err(CheckoutPublicationError::Preserve(error.context(format!(
                    "Checkout WorkspaceIndex CAS 失败且当前状态既不匹配 before 也不匹配 target；事务保留在 {}",
                    transaction.root.display()
                ))));
            }
            Err(read_error) => {
                return Err(CheckoutPublicationError::Preserve(error.context(format!(
                    "Checkout WorkspaceIndex CAS 失败且无法确认当前状态: {read_error:#}；事务保留在 {}",
                    transaction.root.display()
                ))));
            }
        }
...
struct CheckoutPlan {
    ...
    removals: Vec<WorkspaceFileRecord>,
...
fn preflight_workspace(
    ...
    let current_files: BTreeMap<&str, &WorkspaceFileRecord> = current_index
        .files
        .iter()
        .map(|file| (file.path.as_str(), file))
        .collect();
    let target_paths: BTreeSet<&str> = target_tree.iter().map(|file| file.path.as_str()).collect();
...
fn check_removal_conflict(
    repository_root: &Path,
    current: &WorkspaceFileRecord,
    force: bool,
) -> Result<()> {
    ...
    if !force {
            ...
            file_matches_record(&destination, current)?,
...
fn check_target_conflict(
    repository_root: &Path,
    current: Option<&WorkspaceFileRecord>,
    target: &WorkspaceFileRecord,
    force: bool,
) -> Result<bool> {
    ...
    if file_matches_record(&destination, target)? {
        return Ok(false);
    }
    if force {
        return Ok(true);
...
fn file_cache_id(file: &WorkspaceFileRecord) -> Result<String> {
    Ok(describe_manifest(file.total_size, file.chunking, &file.chunks)?.id)
}

fn ensure_file_cache(
    repository: &Repository,
    ...
    match fs::symlink_metadata(&cache_path) {
        Ok(metadata) => {
            ...
            if file_matches_record(&cache_path, file)? {
    ...
    temporary
        ...
        "完整文件缓存大小与 WorkspaceFileRecord 不一致: {}",
        file.path
    );
    ...
    match temporary.persist_noclobber(&cache_path) {
        ...
        Err(error) if error.error.kind() == io::ErrorKind::AlreadyExists => {
                ...
                file_matches_record(&cache_path, file)?,
                "并发创建的完整文件缓存已损坏: {}",
                cache_path.display()
            );
            Ok(cache_path)
        ...
        if !force {
            if let Some(current) = current_files.get(file.path.as_str()) {
                ...
                let destination = workspace_path(repository.root(), &file.path);
                    ...
                    file_matches_record(&destination, file)?,
...
fn ensure_workspace_unchanged(
    ...
    let destination = workspace_path(repository_root, &expected.path);
        ...
        file_matches_record(&destination, expected)?,
...
struct JournalChange {
    ...
    expected_file: Option<WorkspaceFileRecord>,
}
...
impl CheckoutTransaction {
    ...
    fn load(repository: &Repository, root: PathBuf) -> Result<Self> {
        ...
        let original_index: WorkspaceIndex =
            read_transaction_json(&root.join(ORIGINAL_INDEX_FILE_NAME))?;
...
fn remove_published_absent_destination(
    ...
    match fs::symlink_metadata(destination) {
        Ok(metadata) => {
                ...
                metadata.is_file()
                    && !metadata.file_type().is_symlink()
                    && file_matches_record(destination, expected_file)?,
                "Checkout 后目标文件被外部修改，拒绝自动回滚: {}",
                destination.display()
            );
            fs::remove_file(destination)
...
pub(super) fn recover_transaction(
    ...
    let prepared_target = if abort {
        ...
        let files = repository.read_directory_files(&commit.root_directory_id)?;
        let current_index = transaction.original_index.clone();
        let all_paths: BTreeSet<String> = files.iter().map(|file| file.path.clone()).collect();
        let caches = prepare_file_caches(repository, &files, &all_paths)?;
        Some((files, current_index, caches))
    };
