...
use crate::local::model::WorkspaceFileRecord;
...
/// 校验文件与 `WorkspaceFileRecord` 完全一致：大小、逐 Chunk BLAKE3 以及没有多余的尾部内容。
///
/// 路径不存在、类型不是普通文件、是符号链接或内容不一致时返回 `Ok(false)`；只有
/// I/O 失败才返回错误。
pub(crate) fn file_matches_record(path: &Path, file_record: &WorkspaceFileRecord) -> Result<bool> {
    ...
    if !metadata.is_file()
        ...
        || metadata.len() != file_record.total_size
    {
        return Ok(false);
    }
...
mod tests {
    ...
    use super::{file_matches_record, lenient_leaf_metadata, safe_leaf_metadata};
    ...
    fn missing_or_mismatched_files_do_not_match_their_node(
        ...
        assert!(file_matches_record(&path, &node)?);
        assert!(!file_matches_record(
