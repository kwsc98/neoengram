...
//! repository write lock is held while immutable Directory entries and Chunk objects are read so a
...
use crate::local::{
    ...
    model::{ChunkingStrategy, WorkspaceFileRecord},
...
pub(crate) fn materialize_read_only_snapshot(
    ...
    // with add/commit/checkout/gc so GC cannot remove a Chunk after the Directory has been resolved.
    ...
    let (commit_id, files) = resolve_target(repository, target.trim())?;
    ...
    if let Err(error) = materialize_files(repository, &files, temporary.path(), mode) {
...
fn resolve_target(
    repository: &Repository,
    target: &str,
) -> Result<(String, Vec<WorkspaceFileRecord>)> {
...
fn materialize_file(
    ...
    // Keep the final path no-replace even inside the private staging tree.  A malformed Directory or
...
fn validate_destination(repository: &Repository, requested: &Path) -> Result<PathBuf> {
    ensure!(requested.is_absolute(), "只读快照目标必须是绝对路径");
