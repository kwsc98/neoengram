...
pub(crate) struct ExecutedMutation<T> {
    ...
    journal_id: crate::engine::JournalId,
    plan_digest: ContentDigest,
}

impl<T> ExecutedMutation<T> {
    /// Acknowledges the Engine journal only after the caller has published WorkspaceIndex/ref authority.
    ...
    F: FnOnce(&MutationPlan) -> Result<T> + Send,
{
    ...
    let result = execute_mutation(
        ...
        &SystemClock,
        progress,
        &crate::engine::NoopFailureInjector,
    )
    ...
    F: FnOnce(&MutationPlan) -> Result<T> + Send,
{
    fn apply_mutation(
        ...
        for _ in &plan.actions {
            failures.check(crate::engine::FailurePoint::BeforeMutation)?;
        }
        let operation = self
            .operation
            .lock()
...
#[cfg(test)]
mod tests {
    use crate::engine::{JournalId, JournalPhase};

    use super::*;
    use crate::local::{model::WorkspaceIndex, repository::Repository};
    ...
    fn journal_precedes_mutation_and_cas_precedes_finalize() -> Result<()> {
        ...
        executed.publish_and_finalize(|()| {
            ...
            repository
                .write_index_if_version(&WorkspaceIndex::default(), &expected_index_version)?;
            Ok(())
        })?;
        anyhow::ensure!(
            observer
