...
mod tests {
    ...
    use crate::local::model::{Chunk, ChunkingStrategy, Commit, WorkspaceFileRecord};
    use anyhow::Result;
    ...
    fn create_test_mount(cache_bytes: u64) -> Result<TestMount> {
        ...
        let directory_id = repository.store_directory_files(&files)?;
    ...
    fn oversized_whole_file_is_rejected_before_cache_allocation() -> Result<()> {
        ...
        let root = repository.store_directory_files(&[WorkspaceFileRecord {
    ...
    fn directory_pages_resume_after_persistent_ordinals() -> Result<()> {
        ...
        let repository = Repository::open_or_create(repository_root)?;
        repository.initialize_layout()?;
        let files: Vec<WorkspaceFileRecord> = (0..513)
            .map(|index| WorkspaceFileRecord {
    ...
    fn prepared_head_remains_fixed_after_the_branch_advances() -> Result<()> {
        ...
        let first_root = repository.store_directory_files(&[WorkspaceFileRecord {
            path: "first".to_owned(),
            total_size: 0,
            chunking: ChunkingStrategy::FastCdc,
            chunks: Vec::new(),
        ...
        let second_root = repository.store_directory_files(&[WorkspaceFileRecord {
