...
use super::{
    config::{RepositoryConfig, CURRENT_FORMAT_VERSION},
...
const METADATA_DATABASE_FILE_NAME: &str = "metadata.sqlite3";
...
impl Repository {
    ...
    pub(super) fn open(root: PathBuf) -> Result<Self> {
        ...
        let metadata = open_workspace_metadata_store(&metadata_dir, DEFAULT_WORKSPACE_ID);
        let config = RepositoryConfig::from_metadata(metadata.repository_config()?)?;
        ensure!(
        ...
        for root in canonical_start.ancestors() {
            ...
            if neoengram_dir_exists(root)?
                && root
                    .join(NEOENGRAM_DIR_NAME)
                    .join(METADATA_DIR_NAME)
                    .join(METADATA_DATABASE_FILE_NAME)
                    .is_file()
            {
    ...
    pub(crate) fn initialize_layout(&self) -> Result<()> {
        ...
        let metadata_exists = self
            ...
            .join(METADATA_DATABASE_FILE_NAME)
            .try_exists()?;
        let config = if metadata_exists {
            let config = RepositoryConfig::from_metadata(self.metadata_store.repository_config()?)?;
            ensure!(
        ...
        self.metadata_store.initialize(
            ...
            config.format_version,
            object_store_name(config.object_store),
            config.chunking.as_str(),
        )?;
        ...
        let published_config =
            RepositoryConfig::from_metadata(self.metadata_store.repository_config()?)?;
    ...
    pub(crate) fn repository_id(&self) -> Result<String> {
        Ok(self.metadata_store.repository_config()?.repository_id)
    }
}

const fn object_store_name(kind: ObjectStoreKind) -> &'static str {
    match kind {
        ObjectStoreKind::Loose => "loose",
    }
}

fn valid_published_transaction_name(name: &str) -> bool {
    ["checkout-", "rm-", "restore-"].iter().any(|prefix| {
