...
use crate::engine::{
    ...
    CommitPublisher, EngineError, EngineResult, ErrorCode, ProgressSink, PublishCommitRequest,
    PublishCommitResult,
};
use crate::fs::MemoryIndexSnapshot;
use crate::local::{
    metadata::{
        core_file_record, FileRecord, FileSetReader, HeadCas, HeadState, IndexVersion,
        MetadataReader, Page, PageRequest, ReferenceCas, StoredReference,
    },
    model::{Commit, DirectoryEntry, DirectoryEntryKind, WorkspaceFileRecord, WorkspaceIndex},
};
use anyhow::{bail, ensure, Context, Result};
use neoengram_domain::core::{
    CommitId as CoreCommitId, DirectoryEntryKind as CoreDirectoryEntryKind, DirectoryEntryTarget,
};

#[cfg(test)]
use crate::local::metadata::{DirectoryWriter, MetadataStore};
...
/// A materialized WorkspaceIndex and the exact opaque storage version it was read from.
///
/// Keeping the two values together prevents a long-running command from accidentally rereading
/// a newer WorkspaceIndex and combining it with work computed from the original worktree scan.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl VersionedIndex {
    pub(crate) fn index(&self) -> &WorkspaceIndex {
        &self.index
    }
    ...
    pub(crate) fn into_parts(self) -> (WorkspaceIndex, IndexVersion) {
...
impl Repository {
    pub(crate) fn read_index(&self) -> Result<WorkspaceIndex> {
    ...
    /// Read only the opaque WorkspaceIndex version for an end-of-scan consistency check.
    pub(crate) fn current_index_version(&self) -> Result<IndexVersion> {
        Ok(*self.metadata_store.read_index()?.version())
    }

    pub(crate) fn write_index(&self, index: &WorkspaceIndex) -> Result<()> {
        let reader = self.metadata_store.read_index()?;
        let expected = *reader.version();
        drop(reader);
    ...
    /// unreachable objects and the WorkspaceIndex itself is left untouched.
    ...
    #[cfg(test)]
    pub(crate) fn store_directory_files(&self, files: &[WorkspaceFileRecord]) -> Result<String> {
    ...
    /// Builds the current WorkspaceIndex into a canonical Engine graph without publishing HEAD/ref state.
    pub(crate) fn build_commit_graph(
        &self,
        request: &BuildCommitGraphRequest,
        require_nonempty: bool,
        progress: &dyn ProgressSink,
    ) -> Result<BuildCommitGraphResult> {
        let index = self.metadata_store.read_index()?;
        ensure!(
            index.format_version() == crate::local::model::WORKSPACE_INDEX_FORMAT_VERSION,
            "不支持的 index 格式版本 {}",
            index.format_version()
        );
        ensure!(
            index.version() == &request.expected_index_version,
            "WorkspaceIndex 在 Commit 图构造前发生变化"
        );

        let mut page_request = PageRequest::first(STORAGE_SCAN_PAGE_SIZE)?;
        let mut records = Vec::new();
        loop {
            let page = index.scan_files(None, &page_request)?;
            ensure!(
                page.items.len() <= page_request.limit_usize()?,
                "元数据后端返回的 WorkspaceIndex 分页超过请求上限"
            );
            for record in page.items {
                self.verify_index_manifest(&record)?;
                records.push(core_file_record(&record)?);
            }
            let Some(next) = page.next else {
                break;
            };
            ensure!(
                page_request.after.as_deref() != Some(next.as_str()),
                "元数据后端返回了不前进的 WorkspaceIndex 分页游标"
            );
        ...
        let snapshot = MemoryIndexSnapshot::new(*index.version(), records);
        build_commit_graph(request, &snapshot, progress).map_err(Into::into)
    }

    /// Publishes immutable graph objects and then advances the current HEAD through its CAS port.
    pub(crate) fn publish_commit_graph(
        &self,
        request: &PublishCommitRequest,
        expected_head: &RepositoryHead,
    ) -> Result<PublishCommitResult> {
        let publisher = StandaloneCommitPublisher {
            repository: self,
            expected_head,
        };
        publish_commit(request, &publisher).map_err(Into::into)
    }

    fn persist_engine_graph(
        &self,
        graph: &BuildCommitGraphResult,
        expected_head: &RepositoryHead,
    ) -> Result<()> {
        let expected_parent = expected_head
            .commit_id()
            .map(str::parse::<CoreCommitId>)
            .transpose()
            .context("当前 HEAD Commit ID 无效")?;
        ensure!(
            graph.commit.parent == expected_parent,
            "Commit 图父节点与当前 HEAD 不一致"
        );
        for object in &graph.directories {
            let mut writer = self.metadata_store.begin_directory_write()?;
            for entry in &object.directory.entries {
                let (kind, target_id) = match entry.target_id {
                    DirectoryEntryTarget::Manifest(id) => {
                        (DirectoryEntryKind::File, id.to_string())
                    }
                    DirectoryEntryTarget::Directory(id) => {
                        (DirectoryEntryKind::Directory, id.to_string())
                    }
                };
                let expected_kind = match entry.kind {
                    CoreDirectoryEntryKind::File => DirectoryEntryKind::File,
                    CoreDirectoryEntryKind::Directory => DirectoryEntryKind::Directory,
                };
                ensure!(
                    kind == expected_kind,
                    "Engine Directory 类型与 target 不一致"
                );
            ...
            let persisted = writer.finish()?;
            ensure!(
                persisted.id == object.id.to_string(),
                "SQLite publisher 生成了不同的 Directory ID"
            );
        }
        let commit = Commit {
            root_directory_id: graph.commit.root_directory_id.to_string(),
            parent: graph.commit.parent.map(|parent| parent.to_string()),
            message: graph.commit.message.clone(),
            created_at_unix_ms: graph.commit.created_at_unix_ms,
        };
        let commit_id = self.store_commit(&commit)?;
        ensure!(
            commit_id == graph.commit_id.to_string(),
            "SQLite publisher 生成了不同的 Commit ID"
        );
        self.advance_head(expected_head, &commit_id)
    }
    ...
    fn verify_index_manifest(&self, record: &FileRecord) -> Result<()> {
        ...
        let manifest = self.open_manifest(&record.manifest_id)?;
            ...
            "WorkspaceIndex 文件 {} 的大小与 Manifest 不一致",
            record.path
        );
        ensure!(
            manifest.chunk_count() == record.chunk_count,
            "WorkspaceIndex 文件 {} 的 Chunk 数量与 Manifest 不一致",
            record.path
        );
        self.validate_chunking_strategy(manifest.chunking())
            .with_context(|| {
                format!("WorkspaceIndex 文件分块策略不符合仓库配置: {}", record.path)
            })?;
        ...
        loop {
            ...
            "WorkspaceIndex 文件 {} 的 Chunk 数量与 Manifest 分页不一致",
            record.path
        );
        Ok(())
    }

    pub(crate) fn read_directory_files(&self, id: &str) -> Result<Vec<WorkspaceFileRecord>> {
        ...
        let file_count = u64::try_from(files.len()).context("Directory 文件数量超出 u64")?;
        ensure!(
            file_count == expected_file_count,
            "Directory 文件数量与存储记录不一致: {id}"
        );
        validate_files(&files)?;
        let directory_id = self.directory_id(&files)?;
        ensure!(directory_id == id, "Directory 内容与其 ID 不匹配: {id}");
        Ok(files)
    }

    #[cfg(test)]
    pub(crate) fn list_directory_ids(&self) -> Result<Vec<String>> {
    ...
    pub(crate) fn visit_all_workspace_index_files(
        ...
        for workspace in self.list_workspaces()? {
            ...
            total = total
                ...
                .context("全部 Workspace 的 WorkspaceIndex 文件数量溢出")?;
        }
        Ok(total)
    }
    ...
    pub(crate) fn visit_manifest_chunks(
        ...
        manifest_id: &str,
        expected_size: u64,
        mut visitor: impl FnMut(&crate::local::model::Chunk) -> Result<()>,
    ) -> Result<u64> {
...
#[derive(Debug)]
struct StandaloneCommitPublisher<'a> {
    repository: &'a Repository,
    expected_head: &'a RepositoryHead,
}

impl CommitPublisher for StandaloneCommitPublisher<'_> {
    fn publish(&self, request: &PublishCommitRequest) -> EngineResult<PublishCommitResult> {
        self.repository
            .persist_engine_graph(&request.graph, self.expected_head)
            .map_err(|error| {
                EngineError::new(ErrorCode::StorageUnavailable, error.to_string())
                    .with_context("publisher", "standalone_sqlite")
            })?;
...
impl<'a> DirectoryDagBuilder<'a> {
    ...
    fn push(&mut self, record: FileRecord) -> Result<()> {
        ...
        let (file_name, directory_components) =
            components.split_last().context("Directory 路径不能为空")?;
        let common = self
            .open_path
            .iter()
...
) -> Result<Vec<WorkspaceFileRecord>> {
...
mod tests {
    ...
    use crate::local::model::{ChunkingStrategy, WorkspaceFileRecord, WorkspaceIndex};
    use anyhow::Result;
    ...
    fn stale_versioned_index_write_is_rejected() -> Result<()> {
        ...
        let stale_update = WorkspaceIndex {
    ...
    fn identical_subdirectories_are_deduplicated() -> Result<()> {
        ...
        let root_id = repository.store_directory_files(&files)?;
        ...
        assert_eq!(repository.list_directory_ids()?.len(), 2);
        Ok(())
    }
}
