...
fn initialized_store() -> Result<(tempfile::TempDir, SqliteMetadataStore)> {
    ...
    store.initialize("refs/heads/main", &"0".repeat(64), 9, "loose", "fastcdc")?;
    Ok((temporary, store))
}
...
fn canonical_ids_are_stable() -> Result<()> {
    ...
    let core_empty =
        neoengram_domain::core::Manifest::new(0, ChunkingStrategy::FastCdc, Vec::new())?;
    ...
    let core_manifest = neoengram_domain::core::Manifest::new(
        3,
        ChunkingStrategy::FastCdc,
        vec![neoengram_domain::core::ChunkRef::new(
            neoengram_domain::core::ObjectId::for_bytes(b"abc"),
            0,
            3,
        )?],
    )?;
    ...
    let core_directory = neoengram_domain::core::Directory::new(vec![
        neoengram_domain::core::DirectoryEntry::file(
            0,
            neoengram_domain::core::PathComponent::parse("payload.bin")?,
...
fn sqlite_index_version_matches_core_snapshot() -> Result<()> {
    ...
    assert_eq!(
        initial_version,
        neoengram_domain::core::IndexVersion::from_snapshot(0, &[])?
    );
    ...
    assert_eq!(
        committed_version,
        neoengram_domain::core::IndexVersion::from_snapshot(1, &core_records)?
    );
    ...
    assert_eq!(
        deleted_version,
        neoengram_domain::core::IndexVersion::from_snapshot(2, &[core_file_record(&second)?])?
    );
