...
use neoengram_domain::core::{
    canonical::{self, IndexSnapshotHasher},
    ContentDigest,
};
use rusqlite::{params, params_from_iter, types::Value, Connection, OpenFlags, OptionalExtension};

use crate::local::{
    fs::durable::{ensure_or_create_directory, sync_parent},
    model::{
        Chunk, ChunkingStrategy, Commit, DirectoryEntry, DirectoryEntryKind,
        WORKSPACE_INDEX_FORMAT_VERSION,
    },
};

use super::{
    canonical_commit_id, core_file_record, normalize_head_state, validate_directory_entry,
    validate_metadata_object_id, validate_reference_name, validate_reference_prefix,
    DirectoryHasher, DirectoryReader, DirectoryRef, DirectoryWriter, FileRecord, FileSetReader,
    HeadCas, HeadState, ImmutableCatalogStore, IndexReader, IndexTxn, IndexVersion, ManifestHasher,
    ManifestReader, ManifestRef, MetadataReader, MetadataSnapshot, Page, PageRequest, RefStore,
    ReferenceCas, RepositoryConfigRecord, RepositoryMetadataStore, StoredReference, WorkspaceId,
    WorkspaceIndexStore, WorkspaceRecord, WorkspaceRegistry, DEFAULT_WORKSPACE_ID,
};
...
CREATE TABLE repository_state (
    ...
    repository_id TEXT NOT NULL CHECK (
        length(repository_id) = 64 AND repository_id = lower(repository_id)
    ),
    format_version INTEGER NOT NULL CHECK (format_version >= 1),
    object_store TEXT NOT NULL CHECK (object_store <> ''),
    chunking TEXT NOT NULL CHECK (chunking IN ('fastcdc', 'whole-file', 'mixed'))
) STRICT;
...
fn index_record_digest(file: &FileRecord) -> Result<[u8; 32]> {
    Ok(*canonical::index_record_digest(&core_file_record(file)?)?.as_bytes())
}

fn calculate_index_version(
    connection: &Connection,
    workspace_id: &WorkspaceId,
    revision: u64,
) -> Result<(u64, IndexVersion)> {
    let mut statement = connection.prepare(
        "SELECT path, total_size, chunk_count, manifest_id FROM workspace_index_files \
         WHERE workspace_id = ?1 ORDER BY path COLLATE BINARY",
    )?;
    let mut rows = statement.query(params![workspace_id.as_slice()])?;
    let mut hasher = IndexSnapshotHasher::new();
    while let Some(row) = rows.next()? {
        hasher.push(&core_file_record(&file_from_row(row)?)?)?;
    }
    let count = hasher.record_count();
    Ok((count, hasher.finish_version(revision)))
}
...
fn read_index_state(
    ...
    let (format, revision, count, digest_bytes): (i64, i64, i64, Vec<u8>) = connection
        .query_row(
            "SELECT index_format, index_revision, index_count, index_digest \
             FROM workspaces WHERE id = ?1",
            params![workspace_id.as_slice()],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
        )
        .context("无法读取 SQLite WorkspaceIndex 状态")?;
    let format = u32::try_from(format).context("WorkspaceIndex format 超出 u32 范围")?;
    ensure!(
        format == WORKSPACE_INDEX_FORMAT_VERSION,
        "不支持的 WorkspaceIndex 格式版本 {format}（当前支持 {WORKSPACE_INDEX_FORMAT_VERSION}）"
    );
    let revision = decode_u64(revision, "WorkspaceIndex revision")?;
    let count = decode_u64(count, "WorkspaceIndex 文件数量")?;
    let digest = ContentDigest::from_bytes(decode_digest(digest_bytes, "WorkspaceIndex digest")?);
    Ok(StoredIndexState {
        format,
        revision,
        count,
        version: IndexVersion::new(revision, digest),
    })
}
...
impl SqliteIndexTxn<'_> {
    fn base_connection(&self) -> Result<&Connection> {
        self.base_connection
            .as_ref()
            .context("SQLite WorkspaceIndex 事务的 base snapshot 已结束")
    }
    ...
    fn scan_current_files(
        ...
        let retained_limit = limit
            .checked_add(1)
            .context("WorkspaceIndex 事务分页大小溢出")?;
        ...
        let next = has_more.then(|| {
            items
                .last()
                .expect("有后续 WorkspaceIndex 事务页时当前页不可能为空")
                .path
                .clone()
        });
        Ok(Page { items, next })
...
impl IndexTxn for SqliteIndexTxn<'_> {
    ...
    fn delete_prefix(&mut self, prefix: &str) -> Result<u64> {
        ...
        loop {
            ...
            deleted = deleted
                .checked_add(
                    u64::try_from(page.items.len()).context("WorkspaceIndex 删除计数超出 u64")?,
                )
                .context("WorkspaceIndex 删除计数溢出")?;
    ...
    fn commit(mut self: Box<Self>) -> Result<IndexVersion> {
        ...
        run_immediate(&connection, || {
            let current = read_index_state(&connection, &self.store.workspace_id)?;
                ...
                "WorkspaceIndex 在事务执行期间被其他操作更新"
            );
            let mut count = current.count;

            for prefix in &self.deleted_prefixes {
                apply_index_delete(&connection, &self.store.workspace_id, prefix, &mut count)?;
            }
            for file in self.upserts.values() {
                apply_index_upsert(&connection, &self.store.workspace_id, file, &mut count)?;
            }

            let revision = current
                .revision
                .checked_add(1)
                .context("WorkspaceIndex revision 溢出")?;
            let (canonical_count, version) =
                calculate_index_version(&connection, &self.store.workspace_id, revision)?;
            ensure!(
                canonical_count == count,
                "WorkspaceIndex 文件数量与规范 snapshot 不一致"
            );
            connection.execute(
                "UPDATE workspaces SET index_revision = ?1, index_count = ?2, \
                 index_digest = ?3 WHERE id = ?4",
                params![
                    encode_u64(revision, "WorkspaceIndex revision")?,
                    encode_u64(count, "WorkspaceIndex 文件数量")?,
                    version.digest.as_bytes().as_slice(),
...
fn apply_index_delete(
    ...
    let (delete_sql, values) = if prefix.is_empty() {
        (
    ...
    let removed = u64::try_from(connection.execute(&delete_sql, params_from_iter(values))?)
        .context("WorkspaceIndex 删除计数超出 u64")?;
    *count = count
        .checked_sub(removed)
        .context("WorkspaceIndex 文件数量下溢")?;
    Ok(())
}
...
fn apply_index_upsert(
    ...
    let existed = connection
        .query_row(
            "SELECT 1 FROM workspace_index_files \
             WHERE workspace_id = ?1 AND path = ?2",
    ...
    connection.execute(
        ...
        params![
            ...
            file.path,
            encode_u64(file.total_size, "WorkspaceIndex 文件大小")?,
            encode_u64(file.chunk_count, "WorkspaceIndex Chunk 数量")?,
...
impl RepositoryMetadataStore for SqliteMetadataStore {
    fn initialize(
        &self,
        head_reference: &str,
        repository_id: &str,
        ...
        validate_reference_name(head_reference)?;
            ...
            format_version > 0,
            "repository format version must be positive"
        );
        ensure!(!object_store.is_empty(), "repository object store is empty");
        ensure!(
            matches!(chunking, "fastcdc" | "whole-file" | "mixed"),
            "repository chunking policy is invalid"
        );
        ...
        if application_id == 0 && schema_version == 0 {
            ...
            let empty_index_digest = IndexSnapshotHasher::new().finish();
            run_immediate(&connection, || {
                ...
                connection.execute(
                    "INSERT INTO repository_state(\
                         singleton, default_head_reference, repository_id, format_version,\
                         object_store, chunking\
                     ) VALUES (1, ?1, ?2, ?3, ?4, ?5)",
                    params![
                        head_reference,
                        repository_id,
                ...
                connection.execute(
                    "INSERT INTO workspaces(\
                         ...
                         index_format, index_revision, index_count, index_digest\
                     ) VALUES (?1, 'main', '.', 'attached', ?2, NULL, ?3, 0, 0, ?4)",
                    params![
                        DEFAULT_WORKSPACE_ID.as_slice(),
                        head_reference,
                        i64::from(WORKSPACE_INDEX_FORMAT_VERSION),
                        empty_index_digest.as_bytes().as_slice(),
                    ],
                )?;
                Ok(())
            })?;
    ...
    fn repository_config(&self) -> Result<RepositoryConfigRecord> {
        let connection = self.open_existing(true)?;
        connection
            .query_row(
                "SELECT repository_id, format_version, object_store, chunking \
                 FROM repository_state WHERE singleton = 1",
                [],
                |row| {
                    let format_version: i64 = row.get(1)?;
                    Ok(RepositoryConfigRecord {
                        repository_id: row.get(0)?,
                        format_version: u32::try_from(format_version).map_err(|_| {
                            rusqlite::Error::IntegralValueOutOfRange(1, format_version)
                        })?,
                        object_store: row.get(2)?,
                        chunking: row.get(3)?,
                    })
                },
            )
            .context("repository identity is missing from metadata SQLite")
    }
    ...
    fn validate_layout(&self) -> Result<()> {
        ...
        let (default_reference, repository_id, format_version, object_store, chunking): (
            String,
            String,
            i64,
            String,
            ...
            "SELECT default_head_reference, repository_id, format_version, object_store, chunking \
             FROM repository_state WHERE singleton = 1",
            [],
            |row| {
                Ok((
        ...
        validate_reference_name(&default_reference)?;
            ...
            repository_id.len() == 64
                && repository_id
                    .bytes()
                    .all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase()),
            "SQLite 元数据库 repository_id 无效"
        );
        ensure!(format_version > 0, "SQLite 元数据库 format_version 无效");
        ensure!(
            !object_store.is_empty(),
            "SQLite 元数据库 object_store 无效"
        );
        ensure!(
            matches!(chunking.as_str(), "fastcdc" | "whole-file" | "mixed"),
            "SQLite 元数据库 chunking 无效"
        );
...
impl WorkspaceIndexStore for SqliteMetadataStore {
    ...
    fn begin_index_transaction(
        ...
        if let Some(expected) = expected {
                ...
                "WorkspaceIndex 已被其他操作更新，无法基于旧版本提交"
            );
        }
...
impl ImmutableCatalogStore for SqliteMetadataStore {
    ...
    fn put_commit(&self, id: &str, commit: &Commit) -> Result<()> {
            ...
            canonical_commit_id(commit)? == id,
            "Commit ID 与规范内容不一致: {id}"
        );
        let id = decode_id(id)?;
...
fn verify_index(connection: &Connection, workspace_id: &WorkspaceId) -> Result<()> {
    ...
    while let Some(row) = rows.next()? {
        ...
        if let Some(previous) = &previous {
                ...
                previous < &file.path,
                "SQLite WorkspaceIndex 路径未严格递增"
            );
        }
        let core_file = core_file_record(&file)?;
        let stored = decode_digest(row.get(4)?, "WorkspaceIndex record")?;
        ensure!(
            stored == *canonical::index_record_digest(&core_file)?.as_bytes(),
            "WorkspaceIndex record 摘要损坏: {}",
            file.path
        );
        hasher.push(&core_file)?;
        count = count
            .checked_add(1)
            .context("WorkspaceIndex 文件数量溢出")?;
        previous = Some(file.path);
    }
    ensure!(count == state.count, "WorkspaceIndex header 文件数量不一致");
    ensure!(
        hasher.finish() == state.version.digest,
        "WorkspaceIndex digest 与文件内容不一致"
    );
    Ok(())
}
...
fn verify_commits_and_references(session: Rc<ReadSession>) -> Result<()> {
    ...
    loop {
        ...
        for id in &page.items {
            let commit = get_commit_from_connection(&session.connection, id)?
                .with_context(|| format!("Commit 在完整性检查期间消失: {id}"))?;
            ensure!(
                canonical_commit_id(&commit)? == *id,
