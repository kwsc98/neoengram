...
use crate::engine::{
    Clock, ContentAddressedObject, DirectoryMetadata, EngineError, EngineResult, ErrorCode,
    FailureInjector, FailurePoint, FileFingerprint, FileRecord, ImmutableCatalogReader,
    IndexSnapshotReader, IndexVersion, JournalId, JournalRecord, JournalStore, ManifestMetadata,
    ObjectMetadata, ObjectPage, ObjectPutOutcome, ObjectSpec, ObjectStore, Page, PageCursor,
    PageRequest, PathScope, Worktree, WorktreeEntry, WorktreeEntryKind, WorktreeScanRequest,
};
use neoengram_domain::core::{
    ChunkRef, Commit, CommitId, DirectoryEntry, DirectoryId, LogicalPath, ManifestId, ObjectId,
};

use super::objects::copy_and_hash_exact;

#[derive(Debug)]
pub struct MemoryIndexSnapshot {
    version: IndexVersion,
    files: BTreeMap<LogicalPath, FileRecord>,
}

impl MemoryIndexSnapshot {
    #[must_use]
    pub fn new(version: IndexVersion, files: impl IntoIterator<Item = FileRecord>) -> Self {
        Self {
            version,
            files: files
                .into_iter()
...
impl ObjectStore for MemoryObjectStore {
    ...
    fn put_content_addressed(&self, source: &mut dyn Read) -> EngineResult<ContentAddressedObject> {
        ...
        let size = u64::try_from(bytes.len())
            .map_err(|_| EngineError::new(ErrorCode::InvalidArgument, "object size exceeds u64"))?;
        let spec = ObjectSpec::new(ObjectId::for_bytes(&bytes), size);
        let outcome = self.put_from(&spec, &mut Cursor::new(bytes))?;
...
mod tests {
    ...
    fn index_snapshot_uses_stable_keyset_pages() {
        ...
        let entries = ["a", "b", "c"].map(|path| FileRecord {
