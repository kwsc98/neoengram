...
use neoengram_domain::core::CommitId;
use std::sync::Arc;
...
fn create_commit(
    ...
    let parent_directory_id = if let Some(parent_id) = &parent_id {
    ...
    if let Some(parent_directory_id) = parent_directory_id {
        ensure!(
            parent_directory_id != root_directory_id.to_string(),
