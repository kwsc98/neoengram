...
use crate::engine::{JournalStore, ProgressSink};
use crate::fs::FileJournalStore;
...
fn recover_with_engine(
    ...
    if !abort && operation.trim() == "checkout" {
        ...
        // before publishing WorkspaceIndex/HEAD. A second outer mutation would re-enter the same fencing
...
#[cfg(test)]
mod tests {
    use crate::engine::{JournalId, JournalPhase, JournalRecord, MutationIdentity};
