...
use crate::engine::{ProgressEvent, ProgressPhase, ProgressSink, ProgressUnit};
use crate::local::{
    metadata::IndexVersion,
    model::{ChunkingStrategy, WorkspaceFileRecord, WorkspaceIndex},
...
pub(crate) async fn execute_with_options(
    ...
    let tasks = input_files.into_iter().map(|input| {
        ...
        async move {
            ...
            let mut file_record = chunk_file(
                disk_path.clone(),
                task_object_store,
                task_staging_dir,
                selected_chunking,
            ...
            file_record.path = repository_path;
            Ok::<WorkspaceFileRecord, anyhow::Error>(file_record)
        }
    });

    // 等待所有已开始的 spawn_blocking 任务结束，避免某个文件失败后仍有后台写操作。
    // 对象是不可变且内容寻址的，因此失败时留下尚未被 index 引用的对象也是安全的。
    let results: Vec<Result<WorkspaceFileRecord>> = stream::iter(tasks)
...
fn update_index(
    ...
    staged_files: Vec<WorkspaceFileRecord>,
    deletion_scope: Option<&str>,
) -> Result<IndexUpdate> {
    let _lock = repository.acquire_write_lock()?;
    let mut files: BTreeMap<String, WorkspaceFileRecord> = index
        .files
        .into_iter()
    ...
    let removed_files = u64::try_from(before_deletion - files.len())
        .context("WorkspaceIndex 删除文件数量超出 u64 范围")?;
    ...
    let index_version = repository
        ...
        .context("WorkspaceIndex 在 add 扫描期间发生变化，请重试")?;
    Ok(IndexUpdate {
        index_version,
        removed_files,
    })
