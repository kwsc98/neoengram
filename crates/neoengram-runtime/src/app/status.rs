use std::{
    collections::{BTreeMap, BTreeSet},
    path::{Component, Path},
};

use anyhow::{ensure, Context, Result};
use walkdir::WalkDir;

use crate::local::{
    model::WorkspaceFileRecord,
    repository::{is_managed_container_dir_name, is_neoengram_dir_name, Repository},
    worktree::{file_matches_record, lenient_leaf_metadata, workspace_path, IgnoreRules},
};
use crate::{FileChangeKind, HeadPosition, PathChange, StatusResult};

pub(crate) async fn execute(cwd: std::path::PathBuf) -> Result<StatusResult> {
    let repository = Repository::discover(&cwd)?;
    let _worktree_lock = repository.acquire_worktree_shared_lock()?;
    let task_repository = repository.clone();
    let report = tokio::task::spawn_blocking(move || build_report(&task_repository))
        .await
        .context("status 任务异常终止")??;

    Ok(report)
}

fn build_report(repository: &Repository) -> Result<StatusResult> {
    repository.ensure_no_unfinished_transactions()?;
    let ignore_rules = IgnoreRules::load(repository.root())?;
    let versioned_index = repository.read_index_versioned()?;
    let index_version = *versioned_index.version();
    let index = versioned_index.index();
    let head = repository.current_head()?;
    let head_files = match head.commit_id() {
        Some(commit_id) => {
            let commit = repository.read_commit(commit_id)?;
            repository.read_directory_files(&commit.root_directory_id)?
        }
        None => Vec::new(),
    };
    let head_position = if head.is_detached() {
        HeadPosition::Detached(
            head.commit_id()
                .context("Detached HEAD 缺少 Commit")?
                .parse()
                .context("Detached HEAD Commit ID 无效")?,
        )
    } else {
        HeadPosition::Branch(
            head.branch_name()
                .context("Attached HEAD 缺少分支名")?
                .to_owned(),
        )
    };

    let staged = compare_snapshots(&head_files, &index.files);
    let mut unstaged = Vec::new();
    for file in &index.files {
        let path = workspace_path(repository.root(), &file.path);
        match lenient_leaf_metadata(repository.root(), &file.path)? {
            None => unstaged.push(PathChange {
                path: file.path.clone(),
                kind: FileChangeKind::Deleted,
            }),
            Some(metadata)
                if !metadata.is_file()
                    || metadata.file_type().is_symlink()
                    || !file_matches_record(&path, file)? =>
            {
                unstaged.push(PathChange {
                    path: file.path.clone(),
                    kind: FileChangeKind::Modified,
                });
            }
            Some(_) => {}
        }
    }

    let tracked: BTreeSet<&str> = index.files.iter().map(|file| file.path.as_str()).collect();
    let untracked = collect_untracked_files(repository.root(), &tracked, &ignore_rules)?;
    // 共享 worktree 锁冻结工作区事务；WorkspaceIndex/HEAD 的独立 state-only 写入仍可并发，
    // 因此输出前必须复核两者，避免报告混合两个元数据时刻。
    repository.ensure_no_unfinished_transactions()?;
    ensure!(
        repository.current_index_version()? == index_version,
        "WorkspaceIndex 在 status 扫描期间发生变化，请重试"
    );
    ensure!(
        repository.current_head()? == head,
        "HEAD 在 status 扫描期间发生变化，请重试"
    );
    Ok(StatusResult {
        head: head_position,
        staged,
        unstaged,
        untracked,
    })
}

pub(crate) fn is_workspace_clean(repository: &Repository) -> Result<bool> {
    Ok(build_report(repository)?.is_clean())
}

fn compare_snapshots(
    base: &[WorkspaceFileRecord],
    current: &[WorkspaceFileRecord],
) -> Vec<PathChange> {
    let base: BTreeMap<&str, &WorkspaceFileRecord> =
        base.iter().map(|file| (file.path.as_str(), file)).collect();
    let current: BTreeMap<&str, &WorkspaceFileRecord> = current
        .iter()
        .map(|file| (file.path.as_str(), file))
        .collect();
    let paths: BTreeSet<&str> = base.keys().chain(current.keys()).copied().collect();

    paths
        .into_iter()
        .filter_map(|path| {
            let kind = match (base.get(path), current.get(path)) {
                (None, Some(_)) => FileChangeKind::Added,
                (Some(_), None) => FileChangeKind::Deleted,
                (Some(before), Some(after)) if before != after => FileChangeKind::Modified,
                (Some(_), Some(_)) | (None, None) => return None,
            };
            Some(PathChange {
                path: path.to_owned(),
                kind,
            })
        })
        .collect()
}

fn collect_untracked_files(
    repository_root: &Path,
    tracked: &BTreeSet<&str>,
    ignore_rules: &IgnoreRules,
) -> Result<Vec<String>> {
    let tracked_directories = tracked_directory_prefixes(tracked);
    let filter_root = repository_root.to_path_buf();
    let filter_ignore_rules = ignore_rules.clone();
    let managed_container = repository_root
        .join(".neoengram/metadata/metadata.sqlite3")
        .is_file();
    let entries = WalkDir::new(repository_root)
        .follow_links(false)
        .into_iter()
        .filter_entry(move |entry| {
            if is_neoengram_dir_name(entry.file_name()) {
                return false;
            }
            if managed_container
                && entry.depth() == 1
                && is_managed_container_dir_name(entry.file_name())
            {
                return false;
            }
            if !entry.file_type().is_dir() {
                return true;
            }
            let Some(path) = directory_repository_path(entry.path(), &filter_root) else {
                return true;
            };
            path.is_empty()
                || tracked_directories.contains(&path)
                || filter_ignore_rules.has_negations()
                || !filter_ignore_rules.is_ignored(&path, true)
        });
    let mut untracked = Vec::new();
    for entry in entries {
        let entry =
            entry.with_context(|| format!("遍历工作区失败: {}", repository_root.display()))?;
        if !entry.file_type().is_file() {
            continue;
        }
        let path = repository_path(entry.path(), repository_root)?;
        if !tracked.contains(path.as_str()) && !ignore_rules.is_ignored(&path, false) {
            untracked.push(path);
        }
    }
    untracked.sort_unstable();
    Ok(untracked)
}

fn tracked_directory_prefixes(tracked: &BTreeSet<&str>) -> BTreeSet<String> {
    let mut directories = BTreeSet::new();
    for path in tracked {
        let components: Vec<_> = path.split('/').collect();
        let mut current = String::new();
        for component in components.iter().take(components.len().saturating_sub(1)) {
            if !current.is_empty() {
                current.push('/');
            }
            current.push_str(component);
            directories.insert(current.clone());
        }
    }
    directories
}

fn directory_repository_path(path: &Path, repository_root: &Path) -> Option<String> {
    let relative = path.strip_prefix(repository_root).ok()?;
    let mut names = Vec::new();
    for component in relative.components() {
        let Component::Normal(name) = component else {
            return None;
        };
        if is_neoengram_dir_name(name) {
            return None;
        }
        names.push(name.to_str()?);
    }
    Some(names.join("/"))
}

fn repository_path(path: &Path, repository_root: &Path) -> Result<String> {
    let relative = path
        .strip_prefix(repository_root)
        .with_context(|| format!("工作区文件不在仓库根目录内: {}", path.display()))?;
    let mut names = Vec::new();
    for component in relative.components() {
        let Component::Normal(name) = component else {
            anyhow::bail!("工作区路径未规范化: {}", path.display());
        };
        ensure!(
            !is_neoengram_dir_name(name),
            "工作区扫描进入了 NeoEngram 内部路径: {}",
            path.display()
        );
        names.push(
            name.to_str()
                .with_context(|| format!("工作区路径不是有效 UTF-8: {}", path.display()))?,
        );
    }
    ensure!(!names.is_empty(), "工作区文件路径不能为空");
    Ok(names.join("/"))
}

#[cfg(test)]
mod tests {
    use std::fs;

    use super::compare_snapshots;
    use crate::local::model::{Chunk, ChunkingStrategy, WorkspaceFileRecord};
    use crate::local::worktree::lenient_leaf_metadata;
    use crate::{FileChangeKind, PathChange};

    #[test]
    fn snapshot_diff_is_sorted_and_classifies_changes() {
        let before = vec![node("a", "a1"), node("b", "b1"), node("d", "d1")];
        let after = vec![node("a", "a2"), node("b", "b1"), node("c", "c1")];

        assert_eq!(
            compare_snapshots(&before, &after),
            vec![
                PathChange {
                    path: "a".to_owned(),
                    kind: FileChangeKind::Modified,
                },
                PathChange {
                    path: "c".to_owned(),
                    kind: FileChangeKind::Added,
                },
                PathChange {
                    path: "d".to_owned(),
                    kind: FileChangeKind::Deleted,
                },
            ]
        );
    }

    #[test]
    fn parent_file_makes_tracked_descendant_missing() -> Result<(), Box<dyn std::error::Error>> {
        let temporary = tempfile::tempdir()?;
        fs::write(temporary.path().join("a"), b"now a file")?;

        assert!(lenient_leaf_metadata(temporary.path(), "a/b")?.is_none());
        Ok(())
    }

    fn node(path: &str, hash: &str) -> WorkspaceFileRecord {
        WorkspaceFileRecord {
            path: path.to_owned(),
            total_size: 1,
            chunking: ChunkingStrategy::FastCdc,
            chunks: vec![Chunk {
                hash: hash.to_owned(),
                offset: 0,
                size: 1,
            }],
        }
    }
}
