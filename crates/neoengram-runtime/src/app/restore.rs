...
pub(crate) async fn execute(
    ...
    let paths = outcome
        .into_iter()
        .map(neoengram_domain::core::LogicalPath::parse)
        .collect::<Result<Vec<_>, _>>()
        .context("Restore 返回了无效逻辑路径")?;
    Ok(RestoreResult {
        paths,
        target: if staged {
            RestoreTarget::WorkspaceIndex
        } else {
...
fn restore_index(repository: &Repository, scopes: &[String]) -> Result<Vec<String>> {
    ...
    let head_files = current_head_files(repository)?;
    let mut files: BTreeMap<String, WorkspaceFileRecord> = current
        .files
        .into_iter()
...
fn current_head_files(repository: &Repository) -> Result<Vec<WorkspaceFileRecord>> {
