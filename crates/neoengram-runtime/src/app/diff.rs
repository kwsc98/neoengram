...
use crate::local::{
    ...
    model::{Chunk, ChunkingStrategy, WorkspaceFileRecord, WorkspaceIndex},
...
pub(crate) fn build_report(
    ...
    if let Some(expected) = observed_index_version {
            ...
            "WorkspaceIndex 在 diff 扫描期间发生变化，请重试"
        );
    }
...
fn read_head_snapshot(
    repository: &Repository,
) -> Result<(Vec<WorkspaceFileRecord>, TargetObservation)> {
    ...
    let files = match observed.commit_id.as_deref() {
        Some(commit_id) => {
            ...
            repository.read_directory_files(&commit.root_directory_id)?
        }
        None => Vec::new(),
    };
...
/// comparisons, and also by the worktree path scanner after it has built current records.
pub(crate) fn compare_snapshots(
    base: &[WorkspaceFileRecord],
    current: &[WorkspaceFileRecord],
) -> Vec<DiffEntry> {
    let base: BTreeMap<&str, &WorkspaceFileRecord> =
        base.iter().map(|file| (file.path.as_str(), file)).collect();
    let current: BTreeMap<&str, &WorkspaceFileRecord> = current
        .iter()
...
fn file_stats(node: &WorkspaceFileRecord) -> FileStats {
    FileStats {
...
fn chunk_delta(
    before: Option<&WorkspaceFileRecord>,
    after: Option<&WorkspaceFileRecord>,
) -> ChunkDelta {
...
/// Read the relevant regular worktree files into workspace file records. When `tracked_hint` is present (the
...
fn read_worktree_snapshot(
    ...
    if let Some(tracked) = tracked_hint {
        ...
        let managed_container = root.join(".neoengram/metadata/metadata.sqlite3").is_file();
...
fn snapshot_chunking(files: &[WorkspaceFileRecord]) -> BTreeMap<String, ChunkingStrategy> {
    files
        .iter()
...
) -> Result<Option<WorkspaceFileRecord>> {
...
mod tests {
    ...
    use crate::local::model::{Chunk, ChunkingStrategy, WorkspaceFileRecord};
    use crate::FileChangeKind;

    fn node(path: &str, chunks: &[(&str, u64)]) -> WorkspaceFileRecord {
        let mut offset = 0;
        let chunks = chunks
            .iter()
            .map(|(hash, size)| {
