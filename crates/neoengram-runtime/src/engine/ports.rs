...
pub use neoengram_domain::core::{FileRecord, IndexVersion, ObjectSpec};
...
/// Result of publishing a stream whose object identity is computed while it is written.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub struct ContentAddressedObject {
    pub spec: ObjectSpec,
    pub outcome: ObjectPutOutcome,
}
...
pub trait ObjectStore: Debug + Send + Sync {
    ...
    /// Streams an object into store-owned staging while computing its BLAKE3 identity, then
    /// durably publishes it without requiring the caller to read the source twice.
...
pub trait Worktree: Debug + Send + Sync {
    ...
    fn apply_mutation(
        ...
        super::mutation::apply_worktree_actions(plan, self, catalog, objects, progress, failures)
    }
