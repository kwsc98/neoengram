...
use neoengram_domain::core::{ChunkingStrategy, ContentDigest, IndexDelta, Manifest, ObjectId};
...
pub const PREPARED_ADD_HASH_DOMAIN: &[u8] = b"neoengram-runtime-prepared-add-v1";
...
pub struct PrepareAddRequest {
    ...
    pub max_whole_file_bytes: Option<u64>,
...
impl PrepareAddRequest {
    pub fn validate(&self) -> EngineResult<()> {
        ...
        if self.max_whole_file_bytes.is_some()
            && !matches!(
                self.chunking,
                ChunkingSelection::Explicit(ChunkingStrategy::WholeFile)
            )
        {
            return Err(EngineError::new(
                ErrorCode::InvalidArgument,
                "max_whole_file_bytes requires explicit WholeFile chunking",
            ));
        }
        Ok(())
    }
...
impl PreparedAdd {
    ...
    pub fn publication_digest(&self) -> EngineResult<ContentDigest> {
        ...
        Ok(neoengram_domain::core::canonical::add_publication_digest(
