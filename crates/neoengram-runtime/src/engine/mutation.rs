...
use neoengram_domain::core::{ContentDigest, LogicalPath, Manifest, ManifestId, ObjectSpec};
...
pub const MUTATION_PLAN_HASH_DOMAIN: &[u8] = b"neoengram-runtime-mutation-plan-v1";
...
struct ManifestObjectReader<'a> {
    ...
    chunks: std::vec::IntoIter<neoengram_domain::core::ChunkRef>,
    current: Cursor<Vec<u8>>,
}

impl<'a> ManifestObjectReader<'a> {
    fn new(objects: &'a dyn ObjectStore, chunks: Vec<neoengram_domain::core::ChunkRef>) -> Self {
        Self {
            objects,
...
mod tests {
    ...
    use super::*;
    use crate::{
        ContentAddressedObject, DirectoryMetadata, ImmutableCatalogReader, LockManager,
        ManifestMetadata, MutationGuard, NoopFailureInjector, ObjectMetadata, ObjectPage,
        ObjectPutOutcome, Page, WorktreeEntry, WorktreeScanRequest,
    };
    ...
    impl ObjectStore for EmptyObjectStore {
        ...
        fn put_content_addressed(
            &self,
            source: &mut dyn Read,
        ) -> EngineResult<ContentAddressedObject> {
            let mut bytes = Vec::new();
            source.read_to_end(&mut bytes).map_err(EngineError::from)?;
            let size = u64::try_from(bytes.len()).map_err(|_| {
                EngineError::new(ErrorCode::InvalidArgument, "object size exceeds u64")
            })?;
            Ok(ContentAddressedObject {
                spec: ObjectSpec::new(ObjectId::for_bytes(&bytes), size),
                outcome: ObjectPutOutcome::AlreadyPresent,
            })
        }
