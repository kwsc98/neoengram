...
struct CandidateParts {
    ...
    result_digest: neoengram_domain::core::ContentDigest,
...
pub fn prepare_add(
    ...
    validate_whole_file_size_limit(request, &observed)?;
...
fn validate_whole_file_size_limit(
    request: &PrepareAddRequest,
    observed: &BTreeMap<LogicalPath, WorktreeEntry>,
) -> EngineResult<()> {
    let Some(limit) = request.max_whole_file_bytes else {
        return Ok(());
    };
    for (path, entry) in observed {
        let size = entry
        ...
        if size > limit {
            return Err(EngineError::new(
                ...
                "WholeFile input exceeds the StorageVolume max_whole_file_bytes policy",
            )
            .with_context("logical_path", path.to_string())
            .with_context("observed_bytes", size.to_string())
            .with_context("max_whole_file_bytes", limit.to_string()));
        }
    }
    Ok(())
}
...
fn chunk_whole_file(
    ...
    let mut source = worktree.open_file(path)?;
    let published = objects.put_content_addressed(&mut source)?;
    if published.spec.size != expected_size {
        return Err(EngineError::new(
            ErrorCode::WorktreeChangedDuringScan,
    ...
    record_object_outcome(published.outcome, expected_size, statistics)?;
    Ok(vec![ChunkRef::new(published.spec.id, 0, expected_size)?])
}
...
fn index_version_mismatch(
    expected: &neoengram_domain::core::IndexVersion,
    observed: &neoengram_domain::core::IndexVersion,
) -> EngineError {
    EngineError::new(
