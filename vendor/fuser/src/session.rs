...
impl<FS: Filesystem> Session<FS> {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        let transport = if options.contains(&MountOption::AutoUnmount)
        ...
        #[cfg(target_os = "macos")]
        let (ch, mount) = transport;
        #[cfg(not(target_os = "macos"))]
        let (file, mount) = transport;
        #[cfg(not(target_os = "macos"))]
...
impl BackgroundSession {
    ...
    pub fn new<FS: Filesystem + Send + 'static>(se: Session<FS>) -> io::Result<BackgroundSession> {
        ...
        // MFMount must run concurrently with the request loop: completing an FSKit mount requires
        // userspace to receive and answer FUSE_INIT. Preserve spawn_mount2's synchronous mount-error
        // behavior by waiting only after the request thread is live.
        #[cfg(target_os = "macos")]
        if let Some(active_mount) = mount.as_ref() {
            if let Err(error) = active_mount.wait_until_ready() {
                // Dropping the mount closes MFChannel and joins the MFMount worker. Closing the
                // channel wakes the request loop, so it can then be joined without leaking a thread.
                drop(mount);
                let _ = guard.join();
                return Err(error);
            }
