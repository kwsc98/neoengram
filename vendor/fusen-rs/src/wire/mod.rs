...
pub(crate) fn parse_request_control(
    ...
    parse_request_control_with_config(headers, local_timeout, &ServerRequestIdConfig::default())
}

pub(crate) fn parse_request_control_with_config(
    headers: &HeaderMap,
    local_timeout: Duration,
    request_id_config: &ServerRequestIdConfig,
) -> Result<RequestControl, Error> {
    let request_id = match one_header(headers, &REQUEST_ID)? {
        Some(value) => {
            let value = value
                .to_str()
                .map_err(|_| invalid_request_id(request_id_config))?;
            validate_request_id_with_config(value, request_id_config)?;
            value.to_owned()
        }
...
pub(crate) fn validate_request_id(value: &str) -> Result<(), Error> {
    validate_request_id_with_config(value, &ServerRequestIdConfig::default())
}

pub(crate) fn validate_request_id_with_config(
    value: &str,
    config: &ServerRequestIdConfig,
) -> Result<(), Error> {
    if request_id_is_valid_with_config(value, config) {
        Ok(())
    } else {
        Err(invalid_request_id(config))
    }
}

#[cfg(test)]
pub(crate) fn validated_request_id_header(headers: &HeaderMap) -> Option<&str> {
    validated_request_id_header_with_config(headers, &ServerRequestIdConfig::default())
}

pub(crate) fn validated_request_id_header_with_config<'a>(
    headers: &'a HeaderMap,
    config: &ServerRequestIdConfig,
) -> Option<&'a str> {
    ...
    value
        .to_str()
        .ok()
        .filter(|value| request_id_is_valid_with_config(value, config))
}

fn request_id_is_valid(value: &str) -> bool {
    request_id_is_valid_with_config(value, &ServerRequestIdConfig::default())
}

fn request_id_is_valid_with_config(value: &str, config: &ServerRequestIdConfig) -> bool {
    !value.is_empty()
        && value.len() <= config.max_bytes()
        && (!config.require_alphanumeric_prefix()
            || value
                .as_bytes()
                .first()
...
fn invalid_request_id(config: &ServerRequestIdConfig) -> Error {
    let extra = if config.allow_colon() { ", ':'" } else { "" };
    let prefix = if config.require_alphanumeric_prefix() {
        " and begin with an ASCII letter or digit"
    } else {
        ""
    };
    Error::framework(
        ...
        "invalid_request_id",
        format!(
            "x-request-id must be 1-{} ASCII letters, digits, '.', '_' or '-'{}{}",
            config.max_bytes(),
            extra,
            prefix
        ),
...
pub(crate) fn response_headers_without_control(mut headers: HeaderMap) -> HeaderMap {
    for name in [
        CONNECTION,
        CONTENT_TYPE,
        CONTENT_LENGTH,
...
mod tests {
    ...
    fn request_ids_are_strict_ascii_tokens() {
        ...
        let openapi = ServerRequestIdConfig::builder()
            .max_bytes(128)
            .allow_colon(true)
            .require_alphanumeric_prefix(true)
            .build()
            .unwrap();
        assert!(validate_request_id_with_config("issuer:subject", &openapi).is_ok());
        assert!(validate_request_id_with_config(&"a".repeat(128), &openapi).is_ok());
        assert!(validate_request_id_with_config(&"a".repeat(129), &openapi).is_err());
        for invalid in [".request", "_request", "-request", ":request"] {
            assert!(validate_request_id_with_config(invalid, &openapi).is_err());
        }
    }
