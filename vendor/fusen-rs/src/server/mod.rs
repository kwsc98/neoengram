...
use crate::{
    ...
    server::{
        ...
        problem::PreparedProblemEncoder,
...
pub use config::{
    ...
    ServerRequestConfigBuilder, ServerRequestIdConfig, ServerRequestIdConfigBuilder,
};
pub use problem::{EncodedProblem, ProblemEncoder, ProblemEncoding};
...
pub struct ServerBuilder {
    ...
    problem_encoder: Option<Arc<dyn ProblemEncoder>>,
}
    ...
    pub async fn start(self) -> Result<RunningServer, ServerError> {
        ...
        let app = HttpApp::new(
            ...
            HttpAppConfig {
                ...
                problem_encoder: self.problem_encoder,
            },
            self.metrics.clone(),
        );
...
impl ServerBuilder {
    ...
    /// Installs an application-specific RFC 9457 error encoder.
    pub fn problem_encoder(mut self, encoder: impl ProblemEncoder) -> Self {
        self.problem_encoder = Some(Arc::new(encoder));
        self
    }
    ...
    pub fn build(self) -> Result<Server, ServerError> {
        ...
        let problem_encoder = self
            .problem_encoder
            .map(|encoder| {
                PreparedProblemEncoder::new(
                    encoder,
                    self.config.request().max_response_body_bytes(),
                    self.config.request_id().max_bytes(),
                    !self.config.request_id().always_emit(),
                )
                .map_err(|reason| {
                    ServerError::from_message(
                        ServerErrorKind::Validation,
                        format!("invalid custom problem emergency response: {reason}"),
                    )
                })
            })
            .transpose()?;
