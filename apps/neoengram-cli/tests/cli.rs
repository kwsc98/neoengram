...
use support::models::{
    Chunk, ChunkingStrategy, Commit, DirectoryFixture, FileRecordFixture, WorkspaceIndexFixture,
...
fn default_init_uses_sqlite_and_is_idempotent() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repository_config = read_repository_config(&metadata_dir)?;
    assert_eq!(repository_config.1, 9);
    assert_eq!(repository_config.0.len(), 64);
    assert_eq!(repository_config.2, "loose");
    assert_eq!(repository_config.3, "fastcdc");
    ...
    let reopened_config = read_repository_config(&metadata_dir)?;
    assert_eq!(reopened_config.1, 9);
    assert!(metadata_dir.join("metadata.sqlite3").is_file());
    assert!(!metadata_dir.join("index.json").exists());
...
fn whole_file_policy_is_persisted_and_cannot_be_changed() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let metadata_dir = temporary.path().join(".neoengram/metadata");
    let config = read_repository_config(&metadata_dir)?;
    assert_eq!(config.3, "whole-file");
    ...
    let index = read_workspace_index(&metadata_dir)?;
...
fn crashed_new_init_can_be_retried() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        read_workspace_index(&metadata_dir)?,
        WorkspaceIndexFixture::default()
    );
    Ok(())
}
...
fn init_rejects_an_incomplete_existing_repository() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(!neoengram_dir.join("metadata/metadata.sqlite3").exists());
...
fn init_rejects_the_previous_repository_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(
        metadata_dir.join("metadata.sqlite3"),
        b"old repository format",
    )?;
...
fn add_persists_a_normalized_index_and_reuses_objects() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let metadata_dir = temporary.path().join(".neoengram/metadata");
    let first_index = read_workspace_index(&metadata_dir)?;
    ...
    let second = command(temporary.path())
        ...
        .output()?;
    assert!(second.status.success());
    let second_index = read_workspace_index(&metadata_dir)?;
...
fn add_from_a_subdirectory_uses_repository_relative_paths() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let index = read_workspace_index(&temporary.path().join(".neoengram/metadata"))?;
...
fn creates_linear_single_parent_commits() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let first_directory = read_directory_snapshot(&metadata_dir, &first.root_directory_id)?;
    let index = read_workspace_index(&metadata_dir)?;
    assert_eq!(first_directory.files, index.files);
    assert_eq!(first_directory.files.len(), 2);
    ...
    let second_directory = read_directory_snapshot(&metadata_dir, &second.root_directory_id)?;
    let second_paths: Vec<&str> = second_directory
        .files
        .iter()
...
fn detached_checkout_commits_from_the_target_and_can_reattach_main(
    ...
    let restored_index = read_workspace_index(&metadata_dir)?;
    assert_eq!(restored_index.files, first_directory.files);
    ...
    assert_eq!(
        read_workspace_index(&metadata_dir)?.files,
        second_directory.files
    );
...
fn checkout_refuses_dirty_files_before_writing_and_force_restores_them(
    ...
    assert!(command(temporary.path())
        ...
        .success());
    let second_id = current_commit_id(&metadata_dir)?;
    let index_before = read_workspace_index(&metadata_dir)?;
    ...
    assert_eq!(read_workspace_index(&metadata_dir)?, index_before);
    assert_eq!(current_commit_id(&metadata_dir)?, second_id);
...
fn checkout_requires_force_for_staged_or_missing_tracked_files(
    ...
    let index = read_workspace_index(&metadata_dir)?;
    let directory = read_directory_snapshot(&metadata_dir, &head.root_directory_id)?;
    assert_eq!(index.files, directory.files);
    Ok(())
}
...
fn checkout_detects_chunk_corruption_before_mutating_workspace(
    ...
    let z_node = directory
        .files
        .iter()
        .find(|file| file.path == "z.pt")
        .ok_or("missing z.pt in directory")?;
...
fn rejects_paths_outside_the_repository() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let index = read_workspace_index(&repository.path().join(".neoengram/metadata"))?;
...
fn gc_preserves_payloads_for_unreachable_immutable_history(
    ...
    // history. Its immutable Directory must remain fsck-valid until metadata GC exists.
    assert!(command(temporary.path())
        .args(["checkout", &first_id])
        .output()?
        .status
...
fn read_workspace_index(
    metadata_dir: &Path,
) -> Result<WorkspaceIndexFixture, Box<dyn std::error::Error>> {
...
fn read_directory_snapshot(
    metadata_dir: &Path,
    id: &str,
) -> Result<DirectoryFixture, Box<dyn std::error::Error>> {
    let connection = Connection::open(metadata_dir.join("metadata.sqlite3"))?;
    let mut files = Vec::new();
    read_directory_records(&connection, id, "", &mut files)?;
    Ok(DirectoryFixture { files })
}
...
fn read_directory_records(
    ...
    id: &str,
    prefix: &str,
    files: &mut Vec<FileRecordFixture>,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, kind, target_id, total_size) in entries {
        ...
        if kind == 1 {
            ...
            read_directory_records(connection, &target_id, &path, files)?;
        }
    }
    Ok(())
}
...
fn read_repository_config(
    metadata_dir: &Path,
) -> Result<(String, u32, String, String), Box<dyn std::error::Error>> {
    let connection = Connection::open(metadata_dir.join("metadata.sqlite3"))?;
