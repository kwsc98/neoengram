...
use support::models::{
    Chunk, ChunkingStrategy, FileRecordFixture, WorkspaceIndexFixture,
...
fn a_symlinked_lock_file_is_rejected_without_touching_its_target() -> TestResult {
    ...
    assert!(read_workspace_index(repository.path())?.files.is_empty());
    Ok(())
}
...
fn first_indexed_chunk(repository: &Path) -> Result<Chunk, Box<dyn Error>> {
    let index = read_workspace_index(repository)?;
...
fn read_workspace_index(repository: &Path) -> Result<WorkspaceIndexFixture, Box<dyn Error>> {
