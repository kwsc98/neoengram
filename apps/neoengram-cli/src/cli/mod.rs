...
use neoengram_domain::ChunkingStrategy;
use neoengram_runtime::commands::{
...
enum Command {
    ...
    Restore(RestoreArgs),
    /// 删除未被 WorkspaceIndex 或已保存 Directory 引用的 Chunk 对象。
    Gc(GcArgs),
...
enum WorkspaceCommand {
    /// 从固定 Commit 创建一个独立 WorkspaceIndex 的可写 Workspace。
...
pub(crate) async fn run() -> Result<CommandResult> {
    ...
    let _ = neoengram_runtime::install_diagnostic_sink(terminal.clone());
    let _ = neoengram_runtime::install_failure_injector(Arc::new(
...
fn diff_endpoint_label(endpoint: &DiffEndpoint) -> &str {
    match endpoint {
        ...
        DiffEndpoint::WorkspaceIndex => "index",
...
mod tests {
    ...
    use neoengram_domain::{CommitId, LogicalPath};
    use neoengram_runtime::commands::{ExportMode, ExportResult, OutputChannel};
    use neoengram_runtime::{ProgressEvent, ProgressPhase, ProgressUnit};
